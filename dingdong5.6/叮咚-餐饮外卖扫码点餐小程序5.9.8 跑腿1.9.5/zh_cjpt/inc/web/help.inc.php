...
if ($operation == 'display') {
   $list = pdo_getall('cjpt_help',array('uniacid'=>$_W['uniacid']),array() , '' , 'sort ASC');
} elseif ($operation == 'post') {
   $list = pdo_get('cjpt_help',array('id'=>$_GPC['id']));
   if(checksubmit('submit')){  
    ...
    if($_GPC['id']==''){
        ...
        $res = pdo_update('cjpt_help', $data, array('id' => $_GPC['id']));
        if($res){
    ...
    $id=$_GPC['id'];
    $result = pdo_delete('cjpt_help', array('id'=>$id));
    if($result){
