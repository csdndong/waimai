...
//订单列表
	public function doPageJdList(){
		...
		if($_GPC['qs_id']){
			$where.=" and qs_id=:qs_id ";  
			$data[':qs_id']=$_GPC['qs_id'];
		}
		if($lat){
			$sql=" select xx.* from(SELECT *, ROUND(6378.138*2*ASIN(SQRT(POW(SIN(($lat*PI()/180-sender_lat*PI()/180)/2),2)+COS($lat*PI()/180)*COS(sender_lat*PI()/180)*POW(SIN(($lng*PI()/180-sender_lng*PI()/180)/2),2)))*1000) AS juli  
			FROM ".tablename("cjpt_dispatch").$where .") as xx  where xx.juli<={$juli}  ORDER BY xx.juli ASC  ";
		}else{
