...
if($_GPC['time']){
 ...
 if($type=='today'){
  ...
  $where.="  and FROM_UNIXTIME(a.time) LIKE '%{$time}%' ";
}
if($type=='yesterday'){
...
//总数统计
$sql2="select sum(money) as 'total_money' from" . tablename("cjdc_grouporder") ." as a".$where;
...
$list4=pdo_fetch($sql);
$sql="SELECT a.* FROM ".tablename('cjdc_grouporder'). " a".$where." ORDER BY a.id DESC";
$total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('cjdc_grouporder'). " a"  .$where." ORDER BY a.id DESC",$data);
...
if(checksubmit('export_submit', true)) {
  ...
  header("Content-Disposition:attachment; filename=拼团资金数据.csv");
  echo $html;
  exit();
}

include $this->template('web/dlptzjlist');
