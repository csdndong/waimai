...
if($_GPC['op']=='sc'){
  $result = pdo_delete('cjdc_order',array('dn_state'=>3,'uniacid'=>$_W['uniacid']));
    if($result){
...
if(checksubmit('export_submit', true)) {
  ...
  $end=$_GPC['time']['end'];
  $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("cjdc_order")." WHERE uniacid={$_W['uniacid']} and type=2 and time >='{$start}' and time<='{$end}'");
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select a.*,b.name as md_name,c.name as table_name from " . tablename("cjdc_order")."  a"  . " inner join " . tablename("cjdc_store")." b on a.store_id=b.id inner join " . tablename("cjdc_table")." c on a.table_id=c.id WHERE  a.uniacid={$_W['uniacid']} and a.type=2 and a.time >='{$start}' and a.time<='{$end}' limit " . ($j - 1) * 5000 . ",5000 ";
    ...
    for ($i = 0; $i < $size; $i++) {
      ...
      foreach ($buffer as $k =>$row) {
        ...
        for($k=0;$k<count($good);$k++){
          ...
          if($good[$k]['spec']){
            $date6 .=$good[$k]['name'].'('.$good[$k]['spec'].')*'.$good[$k]['number']."  ";
          }else{
            $date6 .=$good[$k]['name'].'*'.$good[$k]['number']."  ";
          } 
        }
        $row['goods']=$date6;
        foreach ($keys as $key) {
