...
if($_GPC['op']=='sc'){
  $result = pdo_delete('cjdc_order',array('dn_state'=>3,'store_id'=>$storeid));
    if($result){
        message('删除成功',$this->createWebUrl('indnorder',array()),'success');
    }else{
        message('删除失败','','error');
...
if(checksubmit('export_submit', true)) {
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select a.*,b.name as md_name,c.name as table_name from " . tablename("cjdc_order")."  a"  . " inner join " . tablename("cjdc_store")." b on a.store_id=b.id inner join " . tablename("cjdc_table")." c on a.table_id=c.id WHERE a.type=2 and a.store_id={$storeid} and a.time >='{$start}' and a.time<='{$end}' limit " . ($j - 1) * 5000 . ",5000 ";
