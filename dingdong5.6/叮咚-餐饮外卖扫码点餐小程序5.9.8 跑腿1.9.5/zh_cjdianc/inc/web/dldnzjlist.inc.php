...
$pagesize=10;
$where=" where a.uniacid=:uniacid and a.type=2 and a.store_id=:store_id and a.pay_type in (1,2) and a.dn_state=2" ;
...
$fw_money= number_format($list4['dn_poundage']/100*($list2['total_money']+$list2['hb_money']),2);
//echo $fw_money;die;
//
if(checksubmit('export_submit', true)) {
  $start=strtotime($_GPC['time']['start']);
  $end=strtotime($_GPC['time']['end']);
  $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("cjdc_order")." WHERE type=2 and store_id={$storeid} and pay_type in (1,2) and dn_state=2 and UNIX_TIMESTAMP(time) >='{$start}' and UNIX_TIMESTAMP(time)<='{$end}'");
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select * from " . tablename("cjdc_order")." WHERE type=2 and store_id={$storeid} and pay_type in (1,2) and dn_state=2 and UNIX_TIMESTAMP(time) >='{$start}' and UNIX_TIMESTAMP(time)<='{$end}' ORDER BY id DESC limit " . ($j - 1) * 5000 . ",5000 ";
    ...
    for ($i = 0; $i < $size; $i++) {
      ...
      foreach ($buffer as $k =>$row) {
        ...
        if($row['pay_type']==1){
          ...
          $row['sh_money']=number_format($row['money']-($list4['dn_poundage']/100*$row['money']),2);
          ...
          $row['ml_money']='0.00';
          $row['tk_money']='0.00';
          $row['fw_money']=number_format(($list4['dn_poundage']/100*$row['money']),2);    
          $row['dn_state']='已入账';
...
include $this->template('web/dldnzjlist');
