...
//总数统计
$sql2="select sum(money) as 'total_money',sum(ps_money) as ps_money,sum(yhq_money2) as hb_money from" . tablename("cjdc_order") ." as a".$where." and pay_type=1";
$list2=pdo_fetch($sql2);
$sql3="select sum(money) as 'total_money',sum(ps_money) as ps_money,sum(yhq_money2) as hb_money from" . tablename("cjdc_order") ." as a".$where." and pay_type=4";
$list3=pdo_fetch($sql3);
$sql5="select sum(money) as 'total_money',sum(ps_money) as ps_money,sum(yhq_money2) as hb_money from" . tablename("cjdc_order") ." as a".$where." and pay_type=2";
...
$jr_wxmoney=$list2['total_money']+$list2['hb_money']-(($list2['total_money']+$list2['hb_money']-$list2['ps_money'])*$list4['poundage']/100)-($list2['ps_money']*$sys['ps_poundage']/100);
$jr_ddmoney=$list3['total_money']+$list3['hb_money']-(($list3['total_money']+$list3['hb_money']-$list3['ps_money'])*$list4['poundage']/100)-($list3['ps_money']*$sys['ps_poundage']/100);
$jr_yemoney=$list5['total_money']+$list5['hb_money']-(($list5['total_money']+$list5['hb_money']-$list5['ps_money'])*$list4['poundage']/100)-($list5['ps_money']*$sys['ps_poundage']/100);
//今日订单统计
$time=date("Y-m-d",time());
...
//总数统计
$sql21="select sum(money) as 'total_money',sum(yhq_money2) as hb_money from" . tablename("cjdc_order") ." as a".$where3." and pay_type=1";
$list21=pdo_fetch($sql21);
$sql31="select sum(money) as 'total_money',sum(yhq_money2) as hb_money from" . tablename("cjdc_order") ." as a".$where3." and pay_type=5";
$list31=pdo_fetch($sql31);
$sql51="select sum(money) as 'total_money',sum(yhq_money2) as hb_money from" . tablename("cjdc_order") ." as a".$where3." and pay_type=2";
$list51=pdo_fetch($sql51);


//var_dump($list2);die;
$dnjr_wxmoney=$list21['total_money']+$list21['hb_money']-(($list21['total_money']+$list21['hb_money'])*$list4['dn_poundage']/100);
$dnjr_ddmoney=$list31['total_money']+$list31['hb_money']-(($list31['total_money']+$list31['hb_money'])*$list4['dn_poundage']/100);
$dnjr_yemoney=$list51['total_money']+$list51['hb_money']-(($list51['total_money']+$list51['hb_money'])*$list4['dn_poundage']/100);
//今日订单统计
...
//总营业额统计
//外卖在线总金额
$wmcost=pdo_get('cjdc_order', array('store_id'=>$storeid,'state '=>array(4,5,10),'pay_type'=>1,'type'=>1), array('sum(money) as total_money','sum(ps_money) as ps_money','sum(yhq_money2) as hb_money'));
//外卖线下支付总金额
$xxcost=pdo_get('cjdc_order', array('store_id'=>$storeid,'state '=>array(4,5,10),'pay_type'=>4,'type'=>1), array('sum(money) as total_money','sum(ps_money) as ps_money','sum(yhq_money2) as hb_money'));
//外卖余额支付总金额
$wmyecost=pdo_get('cjdc_order', array('store_id'=>$storeid,'state '=>array(4,5,10),'pay_type'=>2,'type'=>1), array('sum(money) as total_money','sum(ps_money) as ps_money','sum(yhq_money2) as hb_money'));
//店内在线总金额
$dnwmcost=pdo_get('cjdc_order', array('store_id'=>$storeid,'dn_state '=>2,'pay_type'=>1,'type'=>2), array('sum(money) as total_money','sum(yhq_money2) as hb_money'));
//店内线下支付总金额
$dnxxcost=pdo_get('cjdc_order', array('store_id'=>$storeid,'dn_state '=>2,'pay_type'=>5,'type'=>2), array('sum(money) as total_money','sum(yhq_money2) as hb_money'));
//店内余额支付总金额
$dnyecost=pdo_get('cjdc_order', array('store_id'=>$storeid,'dn_state '=>2,'pay_type'=>2,'type'=>2), array('sum(money) as total_money','sum(yhq_money2) as hb_money'));
//当面在线总金额
...
//在线总数
$zxcost=$wmcost['total_money']+$wmcost['hb_money']+$dnwmcost['total_money']+$dnwmcost['hb_money']+$dmfkcost['total_money']+$ydcost['total_money'];
//$zxcost=$zxcost-$zxcost*$list4['poundage']/100;
$zxsxf=(($wmcost['total_money']+$wmcost['hb_money']-$wmcost['ps_money'])*$list4['poundage']+$wmcost['ps_money']*$sys['ps_poundage']+($dnwmcost['total_money']+$dnwmcost['hb_money'])*$list4['dn_poundage']+$dmfkcost['total_money']*$list4['dm_poundage']+$ydcost['total_money']*$list4['yd_poundage'])/100;
$zxcost=$zxcost-$zxsxf;
$zxcost=number_format($zxcost,2,".", "");
//余额支付总数
$yecost=$wmyecost['total_money']+$wmyecost['hb_money']+$dnyecost['total_money']+$dnyecost['hb_money']+$dmxxcost['total_money']+$ydyecost['total_money'];

$yesxf=(($wmyecost['total_money']+$wmyecost['hb_money']-$wmyecost['ps_money'])*$list4['poundage']+$wmyecost['ps_money']*$sys['ps_poundage']+($dnyecost['total_money']+$dnyecost['hb_money'])*$list4['dn_poundage']+$dmxxcost['total_money']*$list4['dm_poundage']+$ydyecost['total_money']*$list4['yd_poundage'])/100;
$yecost=$yecost-$yesxf;
$yecost=number_format($yecost,2,".", "");
//货到付款支付总数
$xxcost=$xxcost['total_money']+$xxcost['hb_money']+$dnxxcost['total_money']+$dnxxcost['hb_money'];
$xxsxf=(($xxcost['total_money']+$xxcost['hb_money']-$xxcost['ps_money'])*$list4['poundage']+$xxcost['ps_money']*$sys['ps_poundage']+($dnxxcost['total_money']+$dnxxcost['hb_money'])*$list4['dn_poundage'])/100;
$xxcost=$xxcost-$xxsxf;
