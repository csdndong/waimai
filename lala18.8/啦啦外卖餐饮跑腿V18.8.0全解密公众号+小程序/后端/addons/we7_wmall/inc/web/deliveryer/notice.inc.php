...
if ($op == "list") {
    ...
    $params = array(":uniacid" => $_W["uniacid"], ":type" => "delivery");
...
if ($op == "post") {
    ...
    if ($_W["ispost"]) {
        $data = array("uniacid" => $_W["uniacid"], "title" => trim($_GPC["title"]), "content" => htmlspecialchars_decode($_GPC["content"]), "link" => trim($_GPC["link"]), "displayorder" => intval($_GPC["displayorder"]), "status" => intval($_GPC["status"]), "addtime" => TIMESTAMP, "type" => "delivery");
        if (!empty($notice["id"])) {
