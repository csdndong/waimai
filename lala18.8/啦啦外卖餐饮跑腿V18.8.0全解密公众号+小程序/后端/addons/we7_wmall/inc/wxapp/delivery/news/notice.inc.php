...
if ($ta == "list") {
    ...
    $params = array(":uniacid" => $_W["uniacid"], ":agentid" => $deliveryer["agentid"], ":uid" => $_W["deliveryer"]["id"], ":type" => "delivery");
...
if ($ta == "detail") {
    $notice = pdo_get("tiny_wmall_notice", array("id" => $_GPC["id"], "uniacid" => $_W["uniacid"], "status" => 1, "type" => "delivery"));
    if (empty($notice)) {
        imessage(error(0, "该消息不存在或已删除"), "", "ajax");
    }
    pdo_update("tiny_wmall_notice_read_log", array("is_new" => 0), array("notice_id" => $_GPC["id"], "uid" => $_W["deliveryer"]["id"]));
