	...
	diy.getParts = function(){
		diy.parts = {
			background: {
				...
				params: {
					name: '背景图',
					imgurl: '../addons/we7_wmall/plugin/poster/static/img/bg.jpg',
				},
			...
			avatar: {
				name: '商户logo',
				params: {
					avatar: '../addons/we7_wmall/plugin/poster/static/img/head.jpg',
				},
			...
			image: {
				name: '图片',
				params: {
					imgurl: '../addons/we7_wmall/plugin/poster/static/img/image.jpg',
				},
				style: {'top': '10', 'left': '10', 'width': '80', 'height': '80', 'z-index': 1},
			},
			qrcode: {
				name: '二维码',
				params: {
					imgurl: '../addons/we7_wmall/plugin/poster/static/img/qrcode.jpg',
					type: "wechat",
				},
	...
	diy.initSave = function() {
		$(".btn-save").unbind('click').click(function() {
			...
			irequire(['tiny'], function(tiny){
				$.post(tiny.getUrl('poster/store/post'), {id: diy.id, data: diy.data}, function(ret) {
					var ret = ret.message;
					if(ret.errno != 0) {
