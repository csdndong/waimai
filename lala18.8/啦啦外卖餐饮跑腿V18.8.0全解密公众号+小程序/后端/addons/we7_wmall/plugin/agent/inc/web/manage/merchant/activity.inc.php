...
if ($op == "del") {
    ...
    activity_cron();
    imessage(error(0, base64_decode("5rS75Yqo5Yig6Zmk5oiQ5Yqf")), "", "ajax");
}
if ($op == "add") {
    $_W["page"]["title"] = "活动创建";
    ...
    if ($_W["ispost"]) {
        ...
        $title = implode(",", $title);
        $activity = array("uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "title" => $title, "addtime" => TIMESTAMP, "starttime" => $starttime, "endtime" => $endtime, "type" => $type, "status" => 1, "data" => iserializer($data));
