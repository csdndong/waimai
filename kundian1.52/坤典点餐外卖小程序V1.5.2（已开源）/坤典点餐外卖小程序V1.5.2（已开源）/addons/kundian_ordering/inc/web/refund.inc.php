...
switch ($op){
    case 'index':
        ...
        $order_id=$_GPC['id'];
        $orderData=pdo_get('cqkundian_ordering_order',array('uniacid'=>$uniacid,'id'=>$order_id));
        $refundid=refund_create_order1($orderData['order_number'],'kundian_ordering',$orderData['pra_price']);
        ...
        if($result['return_code']=="SUCCESS"){
            ...
            $res2=pdo_update('cqkundian_ordering_order', array('is_pay'=> '5'), array('id' => $order_id));
            if($res1 && $res2){
                message('退款成功',$this->createWebUrl("order"));
            }else{
                message('退款失败',$this->createWebUrl("order"));
            }
        }else{
            message('退款失败1',$this->createWebUrl("order"));
        }
        break;
}
...
function postXmlSSLCurl($xml,$url,$second=30)
{
    ...
    curl_setopt($ch,CURLOPT_SSLCERT,ATTACHMENT_ROOT.'/kundian_ordering/'.$_W['uniacid'].'/apiclient_cert.pem');
    //默认格式为PEM，可以注释
    curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
    curl_setopt($ch,CURLOPT_SSLKEY, ATTACHMENT_ROOT.'/kundian_ordering/'.$_W['uniacid'].'/apiclient_key.pem');
    //post提交方式
