...
class mysql_classcache{
 ...
 function __construct(){ 	 
   ...
   $this->datacachetime = Mysite::$app->config['datacachetime'];  
   $this->datacachelongtime = Mysite::$app->config['datacachelongtime'];  
   $this->uselong = 0;
   $this->connect();
 }
 ...
 function select_one($sql){
	 if($this->uselong == 1){
		 $checkid = md5('lone'.$sql); 
	 }else{
		$checkid = md5('one'.$sql); 
	 }
	$backinfo = memdata::init()->getkey($checkid); 
	if(empty($backinfo)){
		$query=$this->query($sql);
		$backinfo = $this->assoc($query); 
		if($this->uselong == 1){
			memdata::init()->setkey($checkid,$backinfo,$this->datacachelongtime);  
		}else{
			memdata::init()->setkey($checkid,$backinfo,$this->datacachetime); 
		}
		$this->uselong = 0;
	}
 ...
 function getarr($sql){
	  if($this->uselong == 1){
		  $checkid = md5('larr'.$sql);
	  }else{
	$checkid = md5('arr'.$sql);
	  }
	
	$getarr = memdata::init()->getkey($checkid);
	if(empty($backinfo)){
		...
		if($this->uselong == 1){
			memdata::init()->setkey($checkid,$getarr,$this->datacachelongtime);  
		}else{
			memdata::init()->setkey($checkid,$getarr,$this->datacachetime); 
		}
		$this->uselong = 0;
	}
	return $getarr;
