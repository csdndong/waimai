...
{  
    ...
    function upload($savePath, $fileFormat = '', $maxSize = 0, $overwrite = 0 , $fileInput = 'imgFile' , $changeName=1){ 
		$siteconfig = include(hopedir.'config/hopeconfig.php');
        ...
        $this->errno = 0;
		$this->imgserviceurl = ''; 
		if( $siteconfig['is_img_service'] == 1 && !empty($siteconfig['imgserver'])  && $siteconfig['imgserver']  != $siteconfig['siteurl']  ){
			$this->imgserviceurl = $siteconfig['imgserver'];
		}
		
		if(empty($this->imgserviceurl)){
			$this->run($fileInput,$changeName);
		}else{
			$this->outRun($fileInput,$changeName);
		}
 ...
 // 上传
    function outRun($fileInput, $changeName = 1)
    {  
	    $url =   $this->imgserviceurl.'/crossDomainupload.php'; 
		...
		if (isset($_FILES[$fileInput])) {
			...
			if (is_array($fileArr['name'])) { ////上传同文件域名称多个文件 
                ...
                for ($i = 0; $i < count($fileArr['name']); $i++) {
					...
					$tempinfo = $this->out_upload_file($url,$ar['name'],$ar['tmp_name'],$ar['type'],$fileInput,$changeName);
				...
				return $this->errno ? false : true;				
					...
					$tempinfo = $this->out_upload_file($url,$tmpname,$tmpfile,$tmpType,$fileInput,$changeName);
 					 $info = json_decode($tempinfo,true);
   ...
   function out_upload_file($url,$filename,$path,$type,$input_name,$changeName=1){
		try{
    ...
    function copyfile($fileArray)
    {
		...
		if(!$this->imgCheck($fileArray["tmp_name"])){ 
			$this->del($fileArray["tmp_name"]);
			throw new Exception("img fomate error");
		}
        // 文件上传
   ...
   //返回上传单个图片全路径（不含域名）
    function getSigImgDir()
    { 
		if(empty($this->imgserviceurl)){
			return '/'.$this->savePath.$this->returninfo['saveName'];
		}else{
			return $this->returninfo['saveName'];
		}
