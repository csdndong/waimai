...
{
	function updatehuiorder(){
		#print_r(1111);
		$huilist = $this->mysql->getarr("select shopid,id from ".Mysite::$app->config['tablepre']."shophuiorder where id>0");
		#print_r($huilist);exit;
		if(!empty($huilist)){
			for($i=0;$i<count($huilist);$i++){
				$shopinfo = $this->mysql->select_one("select admin_id from ".Mysite::$app->config['tablepre']."shop where id = '".$huilist[$i]['shopid']."'");
				$arr['admin_id'] = $shopinfo['admin_id'];
				$this->mysql->update(Mysite::$app->config['tablepre'].'shophuiorder',$arr,"id='".$huilist[$i]['id']."'");
			}
		}
		$this->success('更新闪惠订单admin_id成功');
	}
	function updatepayname(){
		#print_r(1111);
		$huilist = $this->mysql->getarr("select id from ".Mysite::$app->config['tablepre']."shophuiorder where id>0");
		#print_r($huilist);exit;
		if(!empty($huilist)){
			for($i=0;$i<count($huilist);$i++){
				if($i<=50){
					$arr['paytype_name'] = 'open_acout';
					$this->mysql->update(Mysite::$app->config['tablepre'].'shophuiorder',$arr,"id='".$huilist[$i]['id']."'");
				}else{
					$arr['paytype_name'] = 'weixin';
					$this->mysql->update(Mysite::$app->config['tablepre'].'shophuiorder',$arr,"id='".$huilist[$i]['id']."'");
				}
			}	
		}
		$this->success('更新闪惠订单的支付名称成功');
	}
	function updatepaytime(){
		#print_r(1111);
		$orderlist = $this->mysql->getarr("select addtime,id,paytime from ".Mysite::$app->config['tablepre']."order where id>0");
		if(!empty($orderlist)){
			for($i=0;$i<count($orderlist);$i++){
				if($orderlist[$i]['paytime']<=0){
					$arr['paytime'] = $orderlist[$i]['addtime']+600;
					$this->mysql->update(Mysite::$app->config['tablepre'].'order',$arr,"id='".$orderlist[$i]['id']."'");	
				}
			}
		}
		#print_r(count($orderlist));
		$huilist = $this->mysql->getarr("select addtime,id,paytime from ".Mysite::$app->config['tablepre']."shophuiorder where id>0");
		#print_r($huilist);exit;
		if(!empty($huilist)){
			for($i=0;$i<count($huilist);$i++){
				if($huilist[$i]['paytime']<=0){
					$arr['paytime'] = $huilist[$i]['addtime']+600;
					$this->mysql->update(Mysite::$app->config['tablepre'].'shophuiorder',$arr,"id='".$huilist[$i]['id']."'");
				}				
			}
		}
		$this->success('更新订单支付时间成功');
	}
    function area(){
	...
	function trade_statisyic(){
		#print_r(date('Y-m-d',strtotime('-7 day')));
		#print_r(date('Y-m-d',strtotime('-1 day','2018-06-5')));
		$arealist = $this->mysql->getarr("select adcode,name  from ".Mysite::$app->config['tablepre']."area  limit 0,1000");
...
#print_r($data['datetype']);
		...
		$where2 = '  where addtime > '.strtotime($starttime2.' 00:00:00').' and addtime < '.strtotime($endtime2.' 23:59:59').' ';
		$trendwhere = '';
		$data['shopname'] = '';
		$data['admin_id'] = 0;
		$data['ordertype'] = 0;
		if(!empty($admin_id)){
			...
			$where2 .= ' and admin_id ='.$admin_id;
			$trendwhere .= ' and admin_id ='.$admin_id;
        ...
        $link = IUrl::creatUrl('adminpage/analysis/module/trade_statisyic'.$newlink);
        $data['outlink'] =IUrl::creatUrl('adminpage/analysis/module/outtrade_statisyic'.$newlink);
	...
	function outtrade_statisyic(){
		...
		$data['ordertype'] = '全部';
		if(!empty($admin_id)){
			$stationinfo = $this->mysql->select_one("select stationname from ".Mysite::$app->config['tablepre']."stationadmininfo where cityid= ".$admin_id." ");
...
#print_r($data['datetype']);
        ...
        $newlink = '/starttime/'.$starttime.'/endtime/'.$endtime;
        $where = '  where addtime > '.strtotime($starttime.' 00:00:00').' and addtime < '.strtotime($endtime.' 23:59:59').' ';
		$where2 = '  where addtime > '.strtotime($starttime.' 00:00:00').' and addtime < '.strtotime($endtime.' 23:59:59').' ';
		$data['dno'] = '';
		$data['admin_id'] = 0;
		$data['tradetype'] = 0;
		if(!empty($admin_id)){
		...
		if(!empty($tradetype)){
			...
			$newlink .= '/tradetype/'.$tradetype;
			if($tradetype==2){
				$where .= " and is_reback = 2 ";
			}	
		}
        $link = IUrl::creatUrl('adminpage/analysis/module/trade_log'.$newlink);
        $data['outlink'] =IUrl::creatUrl('adminpage/analysis/module/outtrade_log'.$newlink);
		...
		if(!empty($order1)){
			foreach($order1 as $k=>$val){
				if($val['is_reback']==2){
					...
					$val['drawtime'] = date('Y-m-d H:i:s',$drawinfo['addtime']);//退款成功时间
				}
				if($val['paytype']==0){
	...
	function area_statisyic(){
        ...
        $link = IUrl::creatUrl('adminpage/analysis/module/area_statisyic'.$newlink);
        $data['outlink'] =IUrl::creatUrl('adminpage/analysis/module/outarea_statisyic'.$newlink);

		$pageinfo = new page();
        $pageinfo->setpage(IReq::get('page'));
		$stationlist = $this->mysql->getarr("select name,adcode  from ".Mysite::$app->config['tablepre']."area where id>0 limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."");
		$areadatalist = array();
		$arealist = array();
		if(!empty($stationlist)){
			$new = array();
			foreach($stationlist as $k=>$val){
				$new['name'] = $val['name'];
				$where1 = '';
				if(!empty($val['adcode'])){
					$where1 = $where.' and admin_id ='.$val['adcode'];
					#print_r($where1.'<br>');
				...
				$arealist[] = array(
					'name'=>$new['name'],
					'tradenum'=>$new['data']['tradenum']
				);
				
				$total = $total+$new['data']['tradenum'];
				$data['total'] = $total;
				$data['allshu'] = count($stationlist);
			}
			foreach($areadatalist as $k=>$value){
		...
		array_multisort($tradecost, SORT_DESC,$ordernum, SORT_DESC,$useordcost, SORT_DESC,$useordnum, SORT_DESC, $areadatalist);  
		$data['areadatalist'] = $areadatalist;
		$data['arealist'] = $arealist;
		$shuliang = $this->mysql->counts("select id  from ".Mysite::$app->config['tablepre']."area where id>0 ");
		$pageinfo->setnum($shuliang);
        $data['pagecontent'] = $pageinfo->getpagebar($link);
		#print_r($data['areadatalist']);
        Mysite::$app->setdata($data);
    }
	//导出分站统计
	function outarea_statisyic(){
		...
		$stationlist = $this->mysql->getarr("select name,adcode  from ".Mysite::$app->config['tablepre']."area where id>0 ");
		$areadatalist = array();
		$arealist = array();
		if(!empty($stationlist)){
			$new = array();
		...
		#exit;
        $outexcel = new phptoexcel();
        $titledata = array('排序','站点','订单总数','交易总金额','有效订单数','有效订单金额','下单会员数','单均价');
        $titlelabel = array('sort','name','ordernum','tradecost','useordnum','useordcost','memnum','singlecost');
	...
	function shop_statisyic(){
		...
		$shopnumtype = empty($shopnumtype)?10:$shopnumtype;
		$shopnumtype = $shopnumtype > 50?50:$shopnumtype;
		...
		$newwhere = '  and addtime > '.strtotime($starttime.' 00:00:00').' and addtime < '.strtotime($endtime.' 23:59:59').' ';
		$newlink = '/starttime/'.$starttime.'/endtime/'.$endtime;
		$where2 = ''; 
		$data['starttime'] = $starttime;
        $data['endtime'] = $endtime;
		...
		if(!empty($admin_id)){
			$where2 = ' and admin_id ='.$admin_id;
			$newwhere .= ' and admin_id ='.$admin_id;
			$newlink .= '/admin_id/'.$admin_id;
			$data['admin_id'] = $admin_id;
		}
        //趋势图数据
        ...
        $link = IUrl::creatUrl('adminpage/analysis/module/shop_statisyic'.$newlink);
        $data['outlink1'] =IUrl::creatUrl('adminpage/analysis/module/outshop_statisyic'.$newlink);
		...
		$data['outlink2'] =IUrl::creatUrl('adminpage/analysis/module/outshop_log'.$newlink);
	...
	function outshop_log(){
		...
		$shopdatalist = $this->getrankdata($where,'shop');
		$newshopshuliang = count($shopdatalist);
		$shopnumtype = intval(IReq::get('shopnumtype'));
		$slicenum = 10;
		if($newshopshuliang<10){
	...
	function mem_statisyic(){
		...
		$memnumtype = empty($memnumtype)?10:$memnumtype;
		$memnumtype = $memnumtype > 50?50:$memnumtype;
		...
		if(!empty($admin_id)){
			$where .= ' and admin_id ='.$admin_id;
			$where2 .= ' and admin_id ='.$admin_id;
			$memwhere .=' and admin_id ='.$admin_id;
			$newwhere .= ' and admin_id ='.$admin_id;
			$newlink .= '/admin_id/'.$admin_id;
			$data['admin_id'] = $admin_id;
		}
        ...
        $link = IUrl::creatUrl('adminpage/analysis/module/trade_statisyic'.$newlink);
        $data['outlink1'] =IUrl::creatUrl('adminpage/analysis/module/outmem_statisyic'.$newlink); 
		...
		$data['outlink2'] =IUrl::creatUrl('adminpage/analysis/module/outmem_log'.$newlink);
...
#print_r($data['datetype']);
		...
		$admin_id = intval(IReq::get('admin_id'));
        $newlink = '/starttime/'.$starttime.'/endtime/'.$endtime;
		$jswhere = '  where sj.jstime > '.strtotime($starttime.' 00:00:00').' and sj.jstime < '.strtotime($endtime.' 23:59:59');
		$jswhere2 = '  where sj.jstime > '.strtotime($starttime2.' 00:00:00').' and sj.jstime < '.strtotime($endtime2.' 23:59:59');
		$data['shopname'] = '';
		$data['admin_id'] = 0;
		if(!empty($admin_id)){
			$jswhere .= ' and ord.admin_id ='.$admin_id;
			$jswhere2 .= ' and ord.admin_id ='.$admin_id;
			$newlink .= '/admin_id/'.$admin_id;
			$data['admin_id'] = $admin_id;
		}
		if(!empty($shopname)){			
		...
		#print_r($where);
        $link = IUrl::creatUrl('adminpage/analysis/module/js_statisyic'.$newlink);
        $data['outlink1'] =IUrl::creatUrl('adminpage/analysis/module/outjs_statisyic'.$newlink);
		$data['outlink2'] =IUrl::creatUrl('adminpage/analysis/module/outjs_log'.$newlink);
		//订单总金额
		$orderinfo1 = $this->mysql->select_one("select sum(ord.allcost) as totalcost,sum(ord.cxcost) as allcxcost,sum(ord.shopdowncost) as ptcxcost from ".Mysite::$app->config['tablepre']."shopjs as sj left join  ".Mysite::$app->config['tablepre']."order as ord on sj.orderid = ord.id  ".$jswhere." ");
		$orderinfo2 = $this->mysql->select_one("select sum(ord.allcost) as totalcost,sum(ord.cxcost) as allcxcost from ".Mysite::$app->config['tablepre']."shopjs as sj left join  ".Mysite::$app->config['tablepre']."order as ord on sj.orderid = ord.id  ".$jswhere2." ");
		#print_r($orderinfo1);print_r($orderinfo2);
		...
		#print_r($totalcost1.'yyy');print_r($totalcost2.'yyy');
		...
		$jsinfo1 = $this->mysql->select_one("select sum(sj.acountcost) as jscost,sum(sj.yjcost) as ordyj  from ".Mysite::$app->config['tablepre']."shopjs as sj left join  ".Mysite::$app->config['tablepre']."order as ord on sj.orderid = ord.id  ".$jswhere." ");
		$jsinfo2 = $this->mysql->select_one("select sum(sj.acountcost) as jscost,sum(sj.yjcost) as ordyj  from ".Mysite::$app->config['tablepre']."shopjs as sj left join  ".Mysite::$app->config['tablepre']."order as ord on sj.orderid = ord.id  ".$jswhere2." ");
