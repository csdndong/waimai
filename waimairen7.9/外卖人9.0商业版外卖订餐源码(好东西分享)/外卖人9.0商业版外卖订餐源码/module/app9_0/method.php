...
{ 
	...
	#首页购物车
	function index_cart(){
		$shopids = IReq::get('shopids');//店铺id集--   1136,1139
		$ids = IReq::get('ids');// 常规商品id集--   8960@8943
		$counts = IReq::get('counts');// 常规商品count集--   1,1@1
		$gids = IReq::get('gids');// 规格商品id集--   @8944
		$gcounts = IReq::get('gcounts');// 规格商品count--    @1		
		$idsarr = explode('@',$ids);
		$countsarr = explode('@',$counts); 
		$gidsarr = explode('@',$gids);
		$gcountsarr = explode('@',$gcounts);
		$data = array();
		...
		if(!empty($shopids)){
			...
			foreach($shopidarr as $k=>$v){
				$shipinfo = array();
				$shopinfo = $this->mysql->select_one("select id,shoplogo,shopname,shoptype,goodlistmodule,is_open,starttime from ".Mysite::$app->config['tablepre']."shop where id='".$v."' ");$shopinfo['shoplogo'] = empty($shopinfo['shoplogo'])?Mysite::$app->config['shoplogo']:$shopinfo['shoplogo'];
				$shopinfo['shoplogo'] = getImgQuanDir($shopinfo['shoplogo']);				
				if(!empty($shopinfo)){
					if($shopinfo['shoptype'] == 1){
						$shopdet = $this->mysql->select_one("select limitcost,is_orderbefore,arrivetime from ".Mysite::$app->config['tablepre']."shopmarket where shopid='".$v."' ");   
					} else{
						$shopdet = $this->mysql->select_one("select limitcost,is_orderbefore,arrivetime from ".Mysite::$app->config['tablepre']."shopfast where shopid='".$v."' ");  
					}	
					...
					$monthsellcount  = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."order where shopid='".$v."' and status = 3 and posttime < ".time()." and posttime > ".$mstime." ");
					...
					$shopcost = 0;
					$goodscxdowncost = 0;
					$countsarrx = explode(',',$countsarr[$k]);
					$gcountsarrx = explode(',',$gcountsarr[$k]);
					 
					//常规商品
					if(!empty($idsarr[$k])){
						$goodslist = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."goods  where  id in(".$idsarr[$k].") and shopid = '".$v."' order by field (id,".$idsarr[$k].") ");
						 
						foreach($goodslist as $key=>$value){ 
							 
							if($value['count'] >= $countsarrx[$key]){
								$cxinfo = $this->goodscx($value);
								$value['is_cx'] = $cxinfo['is_cx'];
								$value['oldcost'] = $this->formatcost($value['cost'],2);
								if($value['is_cx'] == 1){
									$havecx  = 1;
								}
								$value['cost'] = $this->formatcost($cxinfo['cxcost'],2);					
								$value['zhekou'] = $cxinfo['zhekou'];
								if($value['is_cx'] == 1){
									$goodscxdowncost += $countsarrx[$key]*($value['oldcost'] - $value['cost']);
								}						
								$shopcost += $value['cost']*$countsarrx[$key];
								$bagcost += $value['bagcost']*$countsarrx[$key]; 
							
								$kk['img'] = empty($value['img'])?getImgQuanDir(Mysite::$app->config['goodlogo']): getImgQuanDir($value['img']);
								$kk['name'] = $value['name'];	
								$kk['attrname'] = '';								
								$kk['cost'] = $this->formatcost($value['cost'],2);
								$kk['oldcost'] = $this->formatcost($value['oldcost'],2);
								$kk['count'] = $countsarrx[$key];
					...
					//规格商品
				    if(!empty($gidsarr[$k])){
						$tempclist =  $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."product where shopid ='".$v."' and id in(".$gidsarr[$k].") order by field (id,".$gidsarr[$k].") ");
						
						foreach($tempclist as $key=>$value){
							
							if($value['stock'] >= $gcountsarrx[$key]){
								$dosee = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."goods where shopid =".$v." and id =".$value['goodsid']."  ");								 
								if(!empty($dosee)){
									$dosee['gg'] = $value;									 
									$dosee['count'] = $gcountsarrx[$key];								   
									$dosee['cost'] = $this->formatcost($value['cost'],2);									
									$cxinfo = $this->goodscx($dosee);
									$dosee['is_cx'] = $cxinfo['is_cx'];
									if($dosee['is_cx'] == 1){
										$havecx  = 1;
									}
									$dosee['cost'] = $this->formatcost($cxinfo['cxcost'],2);
									$dosee['zhekou'] =$cxinfo['zhekou'];
									if($dosee['is_cx'] == 1){
										$goodscxdowncost += intval($gcountsarrx[$key])*($this->formatcost($value['cost'],2) - $dosee['cost']);
									}								
									$shopcost += $dosee['cost']*intval($gcountsarrx[$key]);
									$bagcost += $dosee['bagcost']*intval($gcountsarrx[$key]); 
									$ee['img'] = empty($dosee['img'])?getImgQuanDir(Mysite::$app->config['goodlogo']):getImgQuanDir($dosee['img']);
									$ggname = $value['attrname'];
									$ee['name'] = $dosee['name'];
									$ee['attrname'] = empty($ggname)?'':$ggname;
									$ee['cost'] = $this->formatcost($dosee['cost'],2);
									$ee['oldcost'] = $this->formatcost($value['cost'],2);
									$ee['count'] = intval($gcountsarrx[$key]);
									$ee['is_cx'] = $dosee['is_cx'];
									$ee['id'] = $value['id'];
									$goodsinfo[] = $ee;
								}								 
							}								   
					...
					$shopinfo['bagcost'] = $this->formatcost($bagcost,2);
					$shopinfo['allcost'] = $this->formatcost($shopcost + $bagcost,2);
					$shopinfo['downcost'] = $this->formatcost($goodscxdowncost,2);
	...
	**/	
	function distribution_center(){	   	 		 		 
		...
		$data['cost'] = $this->formatcost($backinfo['fxcost'],2);
		...
		if(empty($backinfo['fxcode'])){
			...
			if(!preg_match('/http:|https:\/\/[\w.]+[\w\/]*[\w.]*\??[\w=&\+\%]*/is',$backinfo['fxcode'])){
				$checkfile = hopedir.$backinfo['fxcode'];
				if(!file_exists($checkfile)){ 
					logwrite("二维码文件不存在,重新构造");		
					$this->makefxcode($backinfo['uid']);
				} 
			}
		}
	...
	function distribution_myyj(){
		...
		$data['cantxcost'] = $this->formatcost($backinfo['fxcost'],2);//可提现金额
	...
	**/
	...
	function waimairenindex(){
		...
		$lat = empty($lat)?0:$lat;
		$lng =empty($lng)?0:$lng;		 
		$data['is_open_weather'] = Mysite::$app->config['is_open_weather'];
		$data['is_open_weather'] = empty($data['is_open_weather'])?0:$data['is_open_weather'];
		...
		if( !empty($adcode) ){
			...
			if( !empty($areacodeone) ){
				...
				$pid = $areacodeone['pid'];
				$areainfoone =  $this->mysqlcache->longTime()->select_one("select * from ".Mysite::$app->config['tablepre']."area where  adcode=".$adcodeid." or adcode=".$pid." order by adcode desc ");
		...
		/**顶部导航自定义按钮 是否显示闪购和跑腿START**/
		$footinfo = array();
		 
		$shangou = $this->mysql->select_one("select is_show from ".Mysite::$app->config['tablepre']."btninfo  where cityid = '".$cityid."' and name = 'shangou' ");		 
		$paotui = $this->mysql->select_one("select is_show from ".Mysite::$app->config['tablepre']."btninfo  where cityid = '".$cityid."' and name = 'paotui' ");		 
		if(empty($shangou)){
			$shangou = array();
			$shangou['is_show'] = 0;
		}
		...
		$footinfo['shangou'] = $shangou;
		$footinfo['paotui'] = $paotui;
		$data['footinfo'] = $footinfo;
		/**顶部导航自定义按钮 是否显示闪购和跑腿END**/
		...
		$Mdata['appadv2'] = $this->mysqlcache->longTime()->getarr("select * from ".Mysite::$app->config['tablepre']."appadv where id > 0 and type =2 and is_show=1 ".$citywhere." order by orderid asc limit 20");
	...
	**/
	function shopsearchgoods(){
		...
		foreach($goodsdet as $k=>$v){
			...
			$v['instro'] = strip_tags($v['instro']);
			$v['oldcost'] = $this->formatcost($v['cost'],2);
			$cxinfo = $this->goodscx($v);
			$v['is_cx'] = $cxinfo['is_cx'];
			if($shopinfo['shoptype'] == 0 && $v['is_cx'] == 1){
				$v['typekey'] = 0;
			} 
			$v['cost'] = $this->formatcost($cxinfo['cxcost'],2);
			$v['zhekou'] = $cxinfo['zhekou'];
			$zhekou = $v['is_cx'] == 1?	$v['zhekou']*0.1:1;
			$v['cxnum'] = intval($cxinfo['cxnum']);
			$v['img'] = empty($v['img'])?getImgQuanDir(Mysite::$app->config['goodlogo']):getImgQuanDir($v['img']);
	...
	function newshop(){ 
		...
		if( !empty($adcode) ){
			...
			if( !empty($areacodeone) ){
				$adcodeid = $areacodeone['id'];
				$pid = $areacodeone['pid'];
				$areainfoone =  $this->mysqlcache->longTime()->select_one("select adcode from ".Mysite::$app->config['tablepre']."area where  adcode=".$adcodeid." or adcode=".$pid." order by adcode desc ");
			}
            if( empty($areainfoone) ){
                $this->message("获取城市失败");
            }else{
	...
	**/
	function marketshop(){
		...
		$lng = trim(IFilter::act(IReq::get('lng')));  
        $adcode = trim(IFilter::act(IReq::get('adcode'))); 
        $shopcat = trim(IFilter::act(IReq::get('shopcat')));  		
		if(empty($adcode)){
		...
		foreach($templist as $k=>$v){
			...
			if(!empty($goodslist)){
				foreach($goodslist as $kk=>$vv){
					...
					$value['is_cx'] = (string)$cxinfo['is_cx'];
					$value['oldcost'] = (string)floatval(round($cxinfo['oldcost'],2));					 
					$value['cost'] = (string)floatval(round($cxinfo['cxcost'],2));					
					$value['zhekou'] =(string)$cxinfo['zhekou'];
					$comlist[] = $value;
				}
			}
			...
			unset($v['sort']);
			unset($v['is_ziti']);
			unset($v['virtualsellcounts']);
			unset($v['pointcount']);
			unset($v['is_open']);			 
			unset($v['pointsellcount']);
			unset($v['lat']);
			unset($v['lng']);
			unset($v['address']);
			unset($v['isforyou']);
			unset($v['is_recom']);			 
			unset($v['is_orderbefore']); 
			unset($v['is_hot']);
			unset($v['is_com']);
			unset($v['is_new']);
			unset($v['pradius']); 
	...
	**/
    function orderprintdata(){
		...
		foreach($orderdet as $key=>$value){
			...
			$value['allcost'] = $this->formatcost($allcost, 2);
			$goods[] = $value;	
		}	
	...
	function getPrintData($orderinfo){
		...
		if($orderinfo['shopps'] > 0){
			$costdetail[] = array('name'=>'配送费','cost'=>$this->formatcost($orderinfo['shopps'],2));
			$new['shopps'] = $orderinfo['shopps'];
		}
		if($orderinfo['bagcost'] > 0){
			$costdetail[] = array('name'=>'打包费','cost'=>$this->formatcost($orderinfo['bagcost'],2));
			$new['bagcost'] = $orderinfo['bagcost'];
		}
		if($orderinfo['addpscost'] > 0){
			$costdetail[] = array('name'=>'附加配送费','cost'=>$this->formatcost($orderinfo['addpscost'],2));
			$new['addpscost'] = $orderinfo['addpscost'];
		}
		if($orderinfo['cxcost'] > 0){
			$costdetail[] = array('name'=>'促销优惠','cost'=>'-'.$this->formatcost($orderinfo['cxcost'],2));
		}
		if($orderinfo['yhjcost'] > 0){
			$costdetail[] = array('name'=>'优惠券优惠','cost'=>'-'.$this->formatcost($orderinfo['yhjcost'],2));
		}
		if($orderinfo['scoredown'] > 0){
			$scoretocost = Mysite::$app->config['scoretocost'];
			$scoretocostmax = $this->formatcost(Mysite::$app->config['scoretocostmax'],2);
			$dx = $this->formatcost(($orderinfo['scoredown']/$scoretocost),2);
		...
		$oldcost = $this->formatcost($oldcost,2);
  ...
  function advimgs(){
	...
	*/
	function appMemlogin(){
		...
		$member= $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."member where uid='".$uid."' ");
		$tjyhj = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."juan where uid='".$uid."' and status < 2 and  endtime > ".time()." "); 
		if(!empty($member['logo'])){ 
	...
	function fastlogin(){
		...
		//if(1 == 1 ){ 
			...
			$tjyhj = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."juan where uid='".$member['uid']."' and status < 2 and  endtime > ".time()." ");
	...
	**/
	...
	function updateuserimg(){
		...
		if($upload->errno!=15&&$upload->errno!=0) {
			...
			$data['logo'] = $filedir;
			// logwrite(var_export($filedir,true).var_export($_POST,true));
	...
	*/
	...
	function appbaidu(){
		...
		$devicetype = trim(IFilter::act(IReq::get('devicetype'))); 
		
		
		
		
		if(strpos($_SERVER['HTTP_USER_AGENT'], 'iPhone') > -1 ||strpos($_SERVER['HTTP_USER_AGENT'], 'iPad')  > -1 ){ 
			$devicetype = 'wmr';
		}else{
			$devicetype = empty($devicetype)?'wmr':$devicetype;
		} 
		if(!empty($userid)){ 
			
			$checkmid =  $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."applogin where userid='".$userid."' ");
			if(empty($checkmid)){//设备ID 不存在
				$Mdata['channelid'] = $channelid;
				$Mdata['userid'] = $userid;
				$Mdata['uid']=$backinfo['uid'];
				$Mdata['addtime'] = time();
				$Mdata['devicetype'] = $devicetype; 
				$this->mysql->insert(Mysite::$app->config['tablepre'].'applogin',$Mdata);  //插入新数据
			}else{ 
				if($checkmid['uid'] != $backinfo['uid']){ 
					$Mdata['channelid'] = $channelid;
					$Mdata['userid'] = $userid;
					$Mdata['uid']=$backinfo['uid'];
					$Mdata['addtime'] = time();
					$Mdata['devicetype'] = $devicetype; 
					$this->mysql->update(Mysite::$app->config['tablepre'].'applogin',$Mdata,"userid='".$userid."'");  
				}
			}
...
//			print_r($value);exit;
    ...
    **/	
	function appwaitorder(){
		...
		foreach($orderlist as $mykey=>$value){
			...
			$orderjscost = $this->getorderjscost($value['id']);
			$value['servicecost'] = $orderjscost['servicecost'];//本单预计收入（结算金额）
			$value['expectincome'] = $orderjscost['expectincome'];	 
	...
	*/
	function goodstype(){
		...
		foreach($shoptype as $key=>$value){
			$value['shuliang'] = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."goods where shopid='".$shopinfo['id']."' and typeid=".$value['id']." order by good_order asc");
			$tempc[] = $value;
		} 
		$this->success($tempc);
	}
	...
	*/
	function orderdetail(){		 
        ...
        if(empty($shopinfo)) $this->message('店铺信息获取失败');
	    $orderjscost = $this->getorderjscost($orderid);
		$data['servicecost'] = $orderjscost['servicecost'];//本单预计收入（结算金额）
		$data['expectincome'] = $orderjscost['expectincome'];	
		...
		$data['pscost'] = $this->formatcost($orderinfo['shopps'],2);//配送费
		$data['cxcost'] = $this->formatcost($orderinfo['cxcost'],2);//促销优惠金额
		$data['jfcost'] = $this->formatcost($orderinfo['scoredown']/Mysite::$app->config['scoretocost'],2);//积分抵扣金额
		$data['yhjcost'] = $this->formatcost($orderinfo['yhjcost'],2);//优惠券抵扣金额
		$data['bagcost'] = $this->formatcost($orderinfo['bagcost'],2);//打包费
		$data['addpscost'] = $this->formatcost($orderinfo['addpscost'],2);//附加配送费
	...
	function newgoodstype(){
		...
		foreach($goodstype as $key=>$value){ 
		   ...
		   foreach($sontypearr as $key1=>$value1){			   	
				$value1['goodscount'] = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."goods where shopid='".$shopinfo['id']."' and typeid   =".$value1['id']."  order by id asc"); 
				$sontype[] = $value1;
	...
	*/
	function newgoodslist(){
		...
		$page = intval(IFilter::act(IReq::get('page')));
		$userAgent = $_SERVER['HTTP_USER_AGENT'];
		if(strpos($userAgent,"iPhone")!==false || strpos($userAgent,"iPad")!==false || strpos($userAgent,"iPod")!==false || strpos($userAgent,"ios")!==false){
			 $pagesize = 1000000; 
		}else{
			$pagesize = intval(IFilter::act(IReq::get('pagesize')));
		}
	...
	function editshop(){
		...
		if($typename == 'shopopentype'){  
			...
			if(empty($shopdet)){
				$data['shopid'] = $shopinfo['id'];
				if($shopinfo['shoptype'] == 0){
				    $this->mysql->insert(Mysite::$app->config['tablepre'].'shopfast',$data);
				}else{
					$this->mysql->insert(Mysite::$app->config['tablepre'].'shopmarket',$data); 
				}
			}else{
				if($shopinfo['shoptype'] == 0){
				    $this->mysql->update(Mysite::$app->config['tablepre'].'shopfast',$data,"shopid='".$shopinfo['id']."'");
...
function applogin(){
	...
	***/
    ...
    function shopcostlog(){
	 ...
	 function shoptxlog(){
	      ...
	      $shuliang  = $this->mysql->counts("select id  from ".Mysite::$app->config['tablepre']."shoptx  where shopuid=".$uid." and type = 0   order by id asc  ");
	...
	function shoptxdetail(){
		...
		if($txinfo['jsid'] > 0){
			...
			if($jsinfo['acountcost'] > 0){
				$info[] = array('name'=>'账户收入','content'=>$this->formatcost($jsinfo['acountcost'],2));
				$info[] = array('name'=>'收入类型','content'=>'订单收入');
			}else{
				$info[] = array('name'=>'账户支出','content'=>$this->formatcost($jsinfo['acountcost'],2));
	...
	*/
	function checkappMem(){
		// logwrite(var_export($_COOKIE,true));
		$uid = trim(IFilter::act(IReq::get('uid')));
		$pwd = trim(IFilter::act(IReq::get('pwd')));
		...
		$wxappoauth= $this->mysql->select_one("select uid from ".Mysite::$app->config['tablepre']."wxappoauth where uid='".$uid."' ");
		$oauth= $this->mysql->select_one("select uid,username,logo from ".Mysite::$app->config['tablepre']."oauth where uid='".$uid."' ");
		# print_r($oauth);
		if(!empty($wxuser) || !empty($wxappoauth)){
			$backarr['is_bdwx'] = 1;
		}
		if(!empty($oauth)){
			$backarr['is_bdqq'] = 1;
			$backarr['username'] = !empty($oauth['username'])?$oauth['username']:$member['username'];
			$backarr['logo'] = $oauth['logo'];
		}
	...
	*/
	...
	function appuserinfo(){
	    ...
	    $tjyhj = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."juan where uid='".$backinfo['uid']."' and status < 2 and  endtime > ".time()." "); 
	...
	*/
	...
	function juan(){
		...
		$nowtime = time();
		$wjuan['shuliang'] =  $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."juan    where uid = ".$backinfo['uid']." and endtime > ".$nowtime." and status = 1 ");
		...
		$ujuan['shuliang'] =  $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."juan    where uid = ".$backinfo['uid']."  and status = 2 ");
		$ujuan['list'] = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."juan where uid = ".$backinfo['uid']."   and status = 2 ");

		$ojuan['shuliang'] =  $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."juan    where uid = ".$backinfo['uid']." and endtime < ".$nowtime." and (status = 1 or status =3)");
...
';
	...
	function newgetshopnew(){
		...
		$shopinfo  = $this->mysql->select_one("select id,goodattrdefault,is_ziti,admin_id,shoptype,is_open,starttime,lat,lng,shopname  from ".Mysite::$app->config['tablepre']."shop  where id = '".$shopid."'    ");
...
/*获取折扣商品*/
		...
		$goodsinfo = array();
		$zcinfo = array();		 
		$zcinfo = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shopzt where shopid='".$shopid."' and is_show = 1 order by sort asc");     
		if(!empty($zcinfo)){ 
			$zcgoodsinfo = array();
			foreach($zcinfo as $k=>$v){
				...
				if(count($goodscount) == 1){
					...
					$vc['goodsinfo']['cost'] = $this->formatcost($vc['goodsinfo']['cost'],2);
			...
			foreach($goodstype as $key=>$value){//id	typeid 商品类型	name 商品名称	count 商品数量	costimg 图片地址	pointbagcost
				...
				if($shopinfo['shoptype'] == 0){ 
					...
					foreach($goodsdet as $k=>$v){
						...
						$v['descgoods'] = empty($v['descgoods'])?'':$v['descgoods'];
						$cxinfo = $this->goodscx($v);
						$v['oldcost'] = $this->formatcost($v['cost'],2);//商品原价
						$v['is_cx'] = $cxinfo['is_cx'];
						$v['cost'] = $this->formatcost($cxinfo['cxcost'],2);//促销价
						$v['zhekou'] = $cxinfo['zhekou'];
						$v['cxnum'] = intval($cxinfo['cxnum']);
	...
	function goodsone(){//获取一个商品信息
		...
		$goodsinfo = $this->mysql->select_one("select  id,shopid,name,cost,is_cx,goodattr,count,typeid,instro,descgoods,virtualsellcount,shopid,sellcount,product_attr,have_det,img from ".Mysite::$app->config['tablepre']."goods where id = ".$goodsid."   order by id asc");
		...
		$goodsinfo['cost'] = $this->formatcost($cxinfo['cxcost'],2);
		...
		if($goodsinfo['have_det'] ==1){
			...
			foreach($productlist as $k=>$v){
				...
				$v['oldcost'] = $v['cost'];
				$v['cost'] = $this->formatcost($cxinfo2['cxcost'],2); 
				...
				if($vatt['id'] > 0){
					...
					if(!empty($detaa)){ 						
						foreach ( $detaa as $keyq=>$valq ){
							...
							$valq['cost'] = $this->formatcost($newdata['cxcost'],2);
	...
	function newshopinfo(){
		...
		$zcinfo = array();
		$data['zcinfo'] = array();
		$zcinfo = $this->mysqlcache->longTime()->getarr("select * from ".Mysite::$app->config['tablepre']."shopzt where shopid='".$shopid."' and is_show = 1 order by sort asc ");     
		foreach($zcinfo as $k=>$v){
			$vc['id'] = $v['id'];
			$vc['shopid'] = $v['shopid'];
			$vc['sort'] = $v['sort'];
		...
		foreach($shoprealcat as $key=>$value){
			$value['imgcount'] = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."shoprealimg where parent_id = ".$value['id']." and img != ''   order by id asc   ");
			$value['imglist'] = array();
	...
	function neworderdet(){
		...
		/*if($backdata['shopcost'] > 0){
			...
			$temp['goodsid'] = 0;
			$temp['goodsname'] = '商品总价';
			$temp['goodscost'] = '¥'.$this->formatcost($backdata['shopcost'],2);
			$temp['goodscount'] = 0;
			$temp['status'] = 0;
			$temp['is_send'] = 0;
			$temp['shopid'] = 0;
		...
		if($backdata['bagcost'] > 0){
			...
			$temp['goodsid'] = 0;
			$temp['goodsname'] = '打包费';
			$temp['goodscost'] = '¥'.$this->formatcost($backdata['bagcost'],2);
			$temp['goodscount'] = 0;
			$temp['status'] = 0;
			$temp['is_send'] = 0;
			$temp['shopid'] = 0;
		...
		if($backdata['shopps'] > 0){
			...
			$temp['goodsid'] = 0;
			$temp['goodsname'] = '配送费';
			$temp['goodscost'] = '¥'.$this->formatcost($backdata['shopps'],2);
			$temp['goodscount'] = 0;
			$temp['status'] = 0;
			$temp['is_send'] = 0;
			$temp['shopid'] = 0;
		...
		if($backdata['addpscost'] > 0){
			...
			$temp['goodscost'] = '¥'.$this->formatcost($backdata['addpscost'],2);//附加配送费
			$temp['goodscount'] = 0;
			$temp['status'] = 0;
			$temp['is_send'] = 0;
			$temp['shopid'] = 0;
		...
		if($backdata['cxcost'] > 0){
			...
			$temp['goodsid'] = 0;
			$temp['goodsname'] = '促销优惠';
			$temp['goodscost'] = '-¥'.$this->formatcost($backdata['cxcost'],2);
			$temp['goodscount'] = 0;
			$temp['status'] = 0;
			$temp['is_send'] = 0;
			$temp['shopid'] = 0;
		...
		if($backdata['yhjcost'] > 0){
			...
			$temp['goodsid'] = 0;
			$temp['goodsname'] = '优惠券';
			$temp['goodscost'] = '-¥'.$this->formatcost($backdata['yhjcost'],2);
			$temp['goodscount'] = 0;
			$temp['status'] = 0;
			$temp['is_send'] = 0;
			$temp['shopid'] = 0;
		...
		if($backdata['scoredowncost'] > 0){
			...
			$temp['goodsid'] = 0;
			$temp['goodsname'] = '积分抵扣';
			$temp['goodscost'] = '-¥'.$this->formatcost($backdata['scoredowncost'],2);
			$temp['goodscount'] = 0;
			$temp['status'] = 0;
			$temp['is_send'] = 0;
			$temp['shopid'] = 0;
		...
		if($backdata['allcost'] > 0){
			...
			$temp['goodsid'] = 0;
			$temp['goodsname'] = '总计';
			$temp['goodscost'] = '¥'.$this->formatcost($backdata['allcost'],2);
			$temp['goodscount'] = 0;
			$temp['status'] = 0;
			$temp['is_send'] = 0;
			$temp['shopid'] = 0;
	   ...
	   /*订单下方操作按钮显示开始*/
	        ...
	        $backdata['kefuphone'] = Mysite::$app->config['litel'];
		$this->success($backdata);
	}
	...
	**/
	function getcx(){ 
		...
		if($jfcost == 'no'){
			...
			$backdata['jfcost'] = empty($jfcost)?0:$this->formatcost($jfcost,2);
		}
		//优惠劵金额
		...
		if($yhjcost == 'no'){
			...
			$backdata['yhjcost'] = empty($yhjcost)?0:$this->formatcost($yhjcost,2);
		}		
		//选择的配送时间段的value值
					   ...
					   if($value['stock'] >= $pidtonum[$value['id']]){
						   ...
						   if(!empty($dosee)){
							   ...
							   $dosee['cost'] = $this->formatcost($value['cost'],2);
								...
								$dosee['zhekou'] =$cxinfo['zhekou'];
								if($dosee['is_cx'] == 1){
                                    $goodscxdowncost += intval($pidtonum[$value['id']])*($this->formatcost($value['cost'],2) - $dosee['cost']);
								}								
								...
								$kk['cost'] = $this->formatcost($dosee['cost'],2);
								$kk['oldcost'] = $this->formatcost($value['cost'],2);
			...
			while($whilestatic >= $nowwhiltcheck){
				...
				foreach($timelist as $key=>$value){
					...
					if($etime  > $checknow){
						...
						$tempt['selfcost'] = $this->formatcost($cost,2) ;
						$tempt['cost'] = $this->formatcost($cost + $pscost,2);
		...
		$backdata['surecost'] = $this->formatcost($backdata['surecost'],2);		
		$backdata['downcost'] =  $this->formatcost($downcost,2);	       	
        $backdata['bagcost'] =  $this->formatcost($backdata['bagcost'],2);		
	...
	function newmakeorder(){
		...
		$shopid = trim(IFilter::act(IReq::get('shopid')));
		$shopinfo  = $this->mysql->select_one("select admin_id,ziti_time,is_open,starttime  from ".Mysite::$app->config['tablepre']."shop  where id = '".$shopid."'    ");
		$adcode = $shopinfo['admin_id'];
		if(!empty($adcode)){
			$this->getplatpsinfo($adcode);
		}
        ...
        $nowhour = strtotime($nowhour);
		$checkinfo = $this->shopIsopen($shopinfo['is_open'],$shopinfo['starttime'],$shopdet['is_orderbefore'],$nowhour);
		if($checkinfo['opentype'] != 2 && $checkinfo['opentype'] != 3 )$this->message('店铺不在营业状态');
		...
		if(count($querids) > 0) {
			...
			foreach($goodslist as $key=>$value){ 
				if($value['count'] >= $myidlist[$value['id']] && $myidlist[$value['id']] > 0){ 
					...
					$value['cost'] = $this->formatcost($value['cost'],2);
					$value['bagcost'] = $this->formatcost($value['bagcost'],2);
					$allcost += $value['cost']*$myidlist[$value['id']];
				   ...
				   foreach($tempclist as $key=>$value){
					    if($value['stock'] >= $pidtonum[$value['id']]){
						   ...
						   if(!empty($dosee)){
							   ...
							   $dosee['cxinfo'] = $value['cxinfo'];
								 if($value['cxinfo']['is_cx'] == 1 ){
										$value['cost'] = $value['cxinfo']['cxcost']; 
								 }
								 $value['cost'] = $this->formatcost($value['cost'],2);
					             $dosee['bagcost'] = $this->formatcost($dosee['bagcost'],2);
		...
		$info['dikou'] = IFilter::act(IReq::get('dikou'));
		$info['platform'] = 4;
		...
		$info['shopinfo'] = $shop;		 
		$info['allcost'] = $this->formatcost($allcost,2);
		$info['bagcost'] = $this->formatcost($bagcost,2);
		$info['allcount'] = $allshu;
		$info['shopps'] = $this->formatcost($shop['pscost'],2);
  ...
  // 'phone' => '18237181260',
	...
	function checkupdate(){
	 ...
	 function testcheckupdate(){
		$apptype = trim(IFilter::act(IReq::get('apptype')));
		if($apptype == 'buyer'){
			$data['appvison'] = '1521025560';//Mysite::$app->config['appvison1'];
			$data['appdownload'] =  'https://m6.waimairen.com/wmr.apk';//Mysite::$app->config['appdownload1'];
		}elseif($apptype == 'seller'){
			$data['appvison'] = Mysite::$app->config['appvison2'];
			$data['appdownload'] =  Mysite::$app->config['appdownload2'];
		}elseif($apptype == 'psuer'){
	...
	function newshoptj(){
		...
		$alltj=  $this->mysql->counts("select  id from ".Mysite::$app->config['tablepre']."order  where shopuid = '".$backinfo['uid']."'  and status = 3  ".$wheredaytime." order by id asc  limit 0,1000");
		...
		$alltj=  $this->mysql->counts("select  id from ".Mysite::$app->config['tablepre']."order  where shopuid = '".$backinfo['uid']."'  and status = 3  ".$whereweektime." order by id asc  limit 0,1000");
		...
		$alltj=  $this->mysql->counts("select  id from ".Mysite::$app->config['tablepre']."order  where shopuid = '".$backinfo['uid']."'  and status = 3  ".$wheremonthtime." order by id asc  limit 0,1000");
...
exit; 
	...
	*/
	function MarketFgoodstype(){
		...
		$tempc = array();
		foreach($shoptype as $key=>$value){ 
			$value['shuliang'] = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."goods where shopid='".$shopinfo['id']."' and typeid in(select id from ".Mysite::$app->config['tablepre']."marketcate where parent_id =".$value['id'].") order by id desc"); 
			$tempc[] = $value;
		} 
...
/* 		$newdata['bagcost'] = intval(IFilter::act(IReq::get('bagcost')));
	...
	*/
	function MarketTgoodstype(){
		...
		$tempc = array();
		foreach($shoptype as $key=>$value){ 
			$value['shuliang'] = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."goods where shopid='".$shopinfo['id']."' and typeid   =".$value['id']."  order by id desc"); 
			$tempc[] = $value;
		}  
		...
		if( !empty($temparr) ){
			foreach($temparr as $key=>$value){
				if( !empty($shopid)  && !empty($shopinfo) ){ 
					...
					$juli = $this->GetDistance($value['lat'], $value['lng'],$shopinfo['lat'], $shopinfo['lng'],2, 2);	
					$juli = ceil($juli);	
...
//账号余额支付  8.6修改
	function appacoutpay(){
		...
		if($this->checksession()){
			...
			if($type == 'order'){//支付订单 
				...
				logwrite('自动接单状态：'.$shopinfo['is_autopreceipt']);
				//获取主单
				/*
				$checktime = time()-15*60;
				if($orderinfo['addtime'] < $checktime){
	...
	function fabupaotui(){
		...
		if($backinfo['uid'] == 0)  $this->message('未登录'); 
    ...
    function getShopRealInfo(){
		...
		$shopRealInfo = array();
		foreach($shoprealcat as $key=>$value){
			$value['imgcount'] = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."shoprealimg where parent_id = ".$value['id']."   order by id asc   ");
			$value['imglist'] = array();
			$imglisttemp = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shoprealimg where parent_id = ".$value['id']." and img != ''  order by id asc   ");
			if( !empty($imglisttemp) ){
				foreach($imglisttemp as $key=>$val){
    ...
    function saveshopreport(){
        ...
        $myreg = '/^1[34578]{1}\d{9}$/';
        if(!IValidate::suremobi($phone))$this->message('手机号码格式错误');
	...
	*	"error": false,
	*	"msg": {
	*		"wxappid": "321321321",【微信app登录appid】 未空时不能使用微信app登录 
	*		"qqappid": "f4243242",【qqapp登录appid】未空时不能使用QQ app登录  
	*	}
	*}   
	*/
	function loginapi(){ 
		...
		if($phonecls->checkcode($phonecode)){
		 ...
		 if($juansetinfo['status'] ==1 && !empty($juaninfo)){
		   ...
		   foreach($juaninfo as $key=>$value){			   
			...
			$tjyhj = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."juan where uid='".$phonemember['uid']."' and status < 2 and  endtime > ".time()." ");
			...
			if(isset($oauthinfo['temp_password'])){
				$oauthinfo['temp_password'] = empty($oauthinfo['temp_password'])?'':$oauthinfo['temp_password'];
			}
			$memberCls = new memberclass($this->mysql);  
			$memberCls->updatememjuaninfo($phone);
	...
	function wxinfo(){
		...
		if($oauthinfo['uid'] > 0){
		    ...
		    if(!empty($checkuserinfo)&& !empty($checkuserinfo['phone'])){
				...
				$tjyhj = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."juan where uid='".$oauthinfo['uid']."' and status < 2 and  endtime > ".time()." ");
				$oauthinfo['juancount'] = $tjyhj;
	...
	function sharesetxxx(){
	 ...
	 function allorder(){
		...
		foreach($orderlist as $key=>$value){
			...
			$checkuid = intval($value['buyeruid']);
			if($checkuid > 0){
				$value['orderNum'] = $this->mysql->counts("select  id from ".Mysite::$app->config['tablepre']."order where buyeruid = ".$value['buyeruid']."  and status = 3 ");
...
//新增函数 
	...
	function lastorderdet(){
		...
		if($orderlist['bagcost'] > 0){
			$temp['goodsname'] = '打包费';
			$temp['goodscost'] = '¥'.$this->formatcost($orderlist['bagcost'],2);			
			$othercostlist[] = $temp;
		}
		if($orderlist['shopps'] > 0){
			$temp['goodsname'] = '配送费';
			$temp['goodscost'] = '¥'.$this->formatcost($orderlist['shopps'],2);			
			$othercostlist[] = $temp;
		}
		if($orderlist['addpscost'] > 0){
			$temp['goodsname'] = '附加配送费';
			$temp['goodscost'] = '¥'.$this->formatcost($orderlist['addpscost'],2);//附加配送费
			$othercostlist[] = $temp;
		}
		if($orderlist['cxcost'] > 0){
			/*
			$temp['goodsname'] = '促销优惠';
			$temp['goodscost'] = '-¥'.$this->formatcost($orderlist['cxcost'],2);
			$othercostlist[] = $temp; */
		}
		if($orderlist['yhjcost'] > 0){
			$temp['goodsname'] = '优惠券';
			$temp['goodscost'] = '-¥'.$this->formatcost($orderlist['yhjcost'],2);
			$othercostlist[] = $temp;
		}
		if($orderlist['scoredowncost'] > 0){
			$temp['goodsname'] = '积分抵扣';
			$temp['goodscost'] = '-¥'.$this->formatcost($orderlist['scoredowncost'],2);
			$othercostlist[] = $temp;
		}
		
		$backdata['othercostlist'] = $othercostlist;
        #$orderlist['cxdet'] = str_replace("exempt","-￥".$this->formatcost($orderlist['shopps'],2),$orderlist['cxdet']);
		$cxdet = unserialize($orderlist['cxdet']);
        $cxdetx = array();
		if(!empty($cxdet)){
			foreach($cxdet as $k=>$v){
				if($v['type'] == 4 && $v['downcost'] === "exempt"){
					$v['downcost'] = "-￥".$this->formatcost($orderlist['shopps'],2);
				}
				$cxdetx[] = $v;
			}
		}
		...
		*/
		...
		if($orderlist['is_ziti'] == 1){//自提单
			...
			if($orderlist['is_reback'] > 0 && $orderlist['status'] != 3 ){	 	
...
//        print_R($shareinfo);       
	...
	function ptorderdet(){
		...
		$orderid = intval(IFilter::act(IReq::get('orderid')));
		$orderinfo = $this->mysql->select_one("select is_ping, id,dno,paytype,paystatus,psuid,psusername,psemail,status,addtime,pttype,is_reback,psstatus,shopname,shopphone,shopaddress,buyeraddress,buyerphone,buyername,buyerlat,buyerlng,shoplat,shoplng,content,movegoodstype,ptkg,movegoodscost,allcost,allkmcost,allkgcost,postdate,farecost,sendtime from ".Mysite::$app->config['tablepre']."order where  buyeruid = ".$backinfo['uid']." and id = '".$orderid."' and shoptype = 100  ");
		#print_r($orderinfo);
		if(empty($orderinfo)){ 
			$this->message('订单不存在');
		}
		$backdata['is_reback'] = $orderinfo['is_reback']  ;
		...
		if($orderinfo['pttype'] == 1){
			...
			$orderinfo['buyerlat'] = $orderinfo['shoplat'] == 0?'':$orderinfo['shoplat'];
	        $orderinfo['buyerlng'] = $orderinfo['shoplng'] == 0?'':$orderinfo['shoplng'];
		}
	...
	function shoploginout(){
		...
		$userid = trim(IFilter::act(IReq::get('userid'))); 
		if(!empty($userid)){ 
			 $this->mysql->delete(Mysite::$app->config['tablepre'].'applogin',"userid ='".$userid."'"); 
		}

		$this->success('操作成功');
	}
	function getorderjscost($orderid){
		$orderinfo = $this->mysql->select_one("select shopid,is_ziti,pstype,shopcost,shopps,bagcost,cxcost,shopdowncost from ".Mysite::$app->config['tablepre']."order where id='".$orderid."' ");
		$shopinfo = $this->mysql->select_one("select zitiyjb,zitilimityj,zitianyj from ".Mysite::$app->config['tablepre']."shop where id='".$orderinfo['shopid']."' ");
		/*获取平台设置的结算公式*/
		...
		if(!empty($orderinfo)){
			if($orderinfo['pstype'] == 1){				
				...
				$orderinfo['servicecost'] = $sjyjnum * $shopinfo['yjin'] * 0.01;//平台服务费（佣金）
				if($orderinfo['is_ziti'] == 1){				
					$ztyj = $sjyjnum * $shopinfo['zitiyjb'] * 0.01;					
					$ztyj = $ztyj < $shopinfo['zitilimityj']?$shopinfo['zitianyj']:$ztyj;
					$servicecost = $ztyj;
				}
				$orderinfo['servicecost'] = round($orderinfo['servicecost'] ,2);
				...
				$orderinfo['expectincome'] = $sjjsnum - $orderinfo['servicecost'];//本单预计收入（结算金额）
				$expectincome = round($orderinfo['expectincome'] ,2);			
			}else{
				//计算平台配送情况下佣金
				...
				$orderinfo['servicecost'] = $ptyjnum * $shopinfo['yjin'] * 0.01;//平台服务费（佣金）
				if($orderinfo['is_ziti'] == 1){
					$ztyj = $ptyjnum * $shopinfo['zitiyjb'] * 0.01;					 
					$ztyj = $ztyj < $shopinfo['zitilimityj']?$shopinfo['zitianyj']:$ztyj;
					$servicecost = $ztyj;
				}
				$orderinfo['servicecost'] = round($orderinfo['servicecost'] ,2);
				...
				$orderinfo['expectincome'] = $ptjsnum - $orderinfo['servicecost'];//本单预计收入（结算金额）
				$expectincome = round($orderinfo['expectincome'] ,2);			
			}	
		}
		$backdata['servicecost'] = $servicecost;
		$backdata['expectincome'] = $expectincome;
		return $backdata;
	}
}