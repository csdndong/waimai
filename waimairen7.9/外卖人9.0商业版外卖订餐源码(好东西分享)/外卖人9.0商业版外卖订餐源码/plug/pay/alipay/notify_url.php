    		...
    		if($backinfog['status'] == 0){
    			 if($backinfog['type'] == 'order'){
					...
					if($backinfog['paystatus'] == 0){
    			 	 ...
    			 	 mysql_query("UPDATE  `".$cfg['tablepre']."order` SET  `paystatus` =  1,`status` =  1,`paytype_name` =  'alipay',`trade_no` =  ".$trade_no.",`paytime` = '".time()."',`addtime` = '".time()."'  where `id`=".$backinfog['upid']."");  
              ...
              $dotime = time(); 
				...
				$checkcost = $memberinfo['cost']+$backinfog['cost'];
					 ...
					 $acountinfo = file_get_contents($cfg['siteurl']."/index.php?ctrl=site&action=acountpayaddlog&dno=".$trade_no);
					   ...
					   mysql_query("UPDATE  `".$cfg['tablepre']."onlinelog` SET  `status` =  1 where `id`=".$out_trade_no." ");  
    			 	  mysql_query("UPDATE  `".$cfg['tablepre']."shophuiorder` SET  `paystatus` =  1,`status` =  1 ,`paytime` = '".time()."',`paytype_name` =  'alipay'  where `id`=".$backinfog['upid']."");   
