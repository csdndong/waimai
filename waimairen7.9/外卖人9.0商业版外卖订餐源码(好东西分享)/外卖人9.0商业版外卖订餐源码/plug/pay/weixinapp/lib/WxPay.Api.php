...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		$newdata = array('error'=>false,'inputdata'=>array()); 
		if($result['return_code'] == FAIL){
			
		}else{
			$newdata['error'] = true; 
	        $inputdata['prepayid'] = $result['prepay_id']; 
			$inputdata['appid'] = WxPayConfig::APPID;
			$inputdata['partnerid'] = WxPayConfig::MCHID;
			$inputdata['package'] = 'Sign=WXPay';
			$inputdata['noncestr'] = self::getNonceStr();
			...
			$string = md5($string);
			//签名步骤四：所有字符转为大写
			$inputdata['sign'] = strtoupper($string);
