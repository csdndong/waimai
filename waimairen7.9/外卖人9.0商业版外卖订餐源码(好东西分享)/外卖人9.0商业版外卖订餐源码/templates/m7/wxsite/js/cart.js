	  ...
	  if(datas.flag == false){
       ...
       if(morepscost>0){ 
		 ...
		 /*================遍历追加优惠活动信息=================*/ 
		    ...
		    $("div[data='exempt']").text('-￥'+datas.content.pscost);   
...
function notusejf(){
		$('.wmr_subord_jf span').html('不使用积分<i class="fa fa-angle-right"></i>');     
		$('.wmr_subord_jf span').css('color','#999');
		$('.jfmaskbg').hide(); 
		$('#jfcost').attr('data',1);
...
function selectjifen(dikoujin,dikoujf,names){
	...
	if(checknext ==  false){ 
	   ...
	   $('.wmr_subord_jf span').html('-￥'+dikoujin+'<i class="fa fa-angle-right"></i>');
	   $('.wmr_subord_jf span').css('color','#ff6e6e');
       $('#jfcost').attr('data',1);
	   $('.jflist').removeClass('navaA'); 
...
function maketime(){
	if(timelist.length > 0){
	  ...
	  $('#DeliveryTime').val(timelist[0].value);
		...
		$('#DeliveryTimesm').text('该商家当前时间不支持配送');
	}
}
/*=================获取配送时间信息列表==================*/
...
function  orderSubmit(){
	...
	if($('.wmr_subord_top h3').text()=='立即配送'){
	 ...
	 if(checknext ==  false){ 
        ...
        $.ajax({         //script定义
                 ...
                 dataType: "json",
                 async:true,
                 data:{shopid:shopid,'remark':remark,'minit':$("#DeliveryTime").val(),'dikou':jfcost,'is_hand':is_hand,'juanid':$('#juanid').val(),'paytype':payway,'buyerlng':buyerlng,'buyerlat':buyerlat},
