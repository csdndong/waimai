			...
			$.each(addresslist, function(i, newobj) { 
				if(newobj.default ==1){
					...
					if(  isexist == 0 ){
						...
						Tmsg('不在平台地图配送范围内');
						ckcanps = '超出配送范围';
						$('input[name="canps"]').val(0);
					}else{
						nopsparghtml = '';						 					
						ckcanps = '';
						$('input[name="canps"]').val(1);
					}  
					...
					var addhtmls = '<p><span id="wmbuyeraddress">'+completeaddress+'</span><span id="ckcanps" style="color:red">'+ckcanps+'</span></p>'	
						          +'<div class="placeorder_head_text">'
								  +'<span id="wmbuyername">'+newobj.contactname+'</span>'
								  +'<span id="wmbuyerphone">'+newobj.phone+'</span>'
								  +'</div></div>'
								  +'<input type="hidden" id="addresslng" value="'+newobj.lng+'" />'
								  +'<input type="hidden" id="addresslat" value="'+newobj.lat+'" />';
					 					  
					$('#memaddress89').html(addhtmls);
						  ...
						  var tempcontent = '<div class="myaddressCon"   onclick="edite_myaddress(this)"   id_data="" contactname="" tag="" sex="" phone=""  lat=""  lng=""   bigadr="点击选择地址"   detailadr=""   add_new=""  > <div class="myaddressBox"><i>添加收货地址</i></div> </div> ';
 						   ...
 						   for(var i=0;i<content.msg.length;i++){
						       var tempinfo = content.msg[i];
								...
								//console.log($("#defaultaddt_"+tempinfo.id).attr('data-is_exist'));
							   if($("#defaultaddt_"+tempinfo.id).attr('data-is_exist') == 0){
								   $("#defaultaddt_"+tempinfo.id).find('.defaultaddt').removeAttr('onclick');
							   }
						   }						   
