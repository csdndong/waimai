	...
	public function doPageDmOrder() {
		...
		if ($res) {
			if ($_GPC['pay_type'] == 2) {
				...
				$this->getapi("" . $_W['siteroot'] . "app/index.php?i=" . $_W['uniacid'] . "&c=entry&a=wxapp&do=sms&m=zh_cjdianc&type=2&store_id=" . $_GPC['store_id']); //短信
				$this->getapi("" . $_W['siteroot'] . "app/index.php?i=" . $_W['uniacid'] . "&c=entry&a=wxapp&do=QtPrint&m=zh_cjdianc&order_id=" . $order_id); //打印机
				$this->getapi("" . $_W['siteroot'] . "app/index.php?i=" . $_W['uniacid'] . "&c=entry&a=wxapp&do=addintegral&m=zh_cjdianc&type=5&order_id=" . $order_id); //短信
			}
			echo $order_id;
		} else {
			echo json_encode('下单失败');
...
//服务商退款
					...
					$path_cert = IA_ROOT . "/addons/zh_cjdianc/cert/" . 'apiclient_cert' . $_W['uniacid'] . '.pem';
					$path_key = IA_ROOT . "/addons/zh_cjdianc/cert/" . 'apiclient_key' . $_W['uniacid'] . '.pem';
					$appid = $system['appid'];
					$key = $res['wxkey'];
					$mchid = $res['mchid'];
				}
...
//测试微信卡券--->获取access_token
	public function wechatCardAccessToken() {
...
//测试微信卡券--->创建卡券
	public function doPageWechatCard() {
		global $_W, $_GPC;
