	...
	public function getMainMenu() {
		...
		if ($_W['role'] == 'operator') {
			...
			$navarr = array(
				...
				3 => $this->createMainMenu('服务商支付', $do, 'servicepay', ''),
				...
				9 => $this->createMainMenu('公众号提醒', $do, 'wetemplate', ''),
...
//服务商退款
			$path_cert = IA_ROOT . "/addons/zh_cjdianc/cert/" . 'apiclient_cert_service' . $_W['uniacid'] . '.pem';
			$path_key = IA_ROOT . "/addons/zh_cjdianc/cert/" . 'apiclient_key_service' . $_W['uniacid'] . '.pem';
			$appid = $res2['appid'];
			$key = $res2['wxkey'];
			$mchid = $res2['mchid'];
			$sub_appid = $res['appid'];
...
//*********************配置项*************************
	...
	public function rushPrint($order_id) {
		//前台打印
...
//喜讯
	...
	public function groupPrint($order_id) {
		//前台打印

		global $_W, $_GPC;
		include IA_ROOT . '/addons/zh_cjdianc/print/dyj.php';
		$order = pdo_get('cjdc_grouporder', array('id' => $order_id));
		$res = pdo_get('cjdc_order', array('id' => $_GPC['order_id']));
		$store = pdo_get('cjdc_store', array('id' => $order['store_id']), 'name');
		$res3 = pdo_getall('cjdc_dyj', array('store_id' => $order['store_id'], 'state' => 1, 'location' => 1));
		...
		foreach ($res3 as $key => $value) {
			...
			$content .= "          " . $store['name'] . $style . $style;
			$content .= "         订单编号  #" . $order['id'] . $style . $style;
			$content .= "           拼团订单  " . $style . $style;
			$content .= "          " . $value['dyj_title'] . $style . $style;
			$content .= "------------" . $is_yue . "------------" . $style;
			$content .= "--------------------------------" . $style;
			$content .= "下单时间：" . date('Y-m-d H:i:s', $order['time']) . $style;
			$content .= "--------------------------------" . $style;

			$content .= '名称' . str_repeat(" ", 15) . "数量  价格" . $style;
			$content .= "--------------------------------" . $style;
			$content .= "" . $order['goods_name'] . "$style";
			$content .= str_repeat(" ", 20) . $order['goods_num'] . "   " . $order['money'] . $style;
			$content .= "--------------------------------" . $style;
			$content .= "流水号：" . $order['order_num'] . $style;
			$content .= "送货地点：" . $order['receive_address'] . $style;
			$content .= "联系电话：" . $order['receive_tel'] . $style;
			$content .= "联系人：" . $order['receive_name'] . $style;

			if ($order['note']) {
				$content .= "备注：" . $order['note'] . $style;
			}
...
//喜讯
	...
	public function storePoundage($storeid) {
		$sql = "select b.dm_poundage,b.dn_poundage,b.poundage,b.yd_poundage from" . tablename('cjdc_store') . "a  left join " . tablename('cjdc_storetype') . " b on a.md_type=b.id where a.id={$storeid}";
		$list = pdo_fetch($sql);
