...
if($_GPC['id']){
	...
	if($res){
		message('设置成功',$this->createWebUrl2('dlgroupgoods',array('page'=>$_GPC['page'],'keywords'=>$_GPC['keywords'],'dishes_type'=>$_GPC['dishes_type'],'type_id'=>$_GPC['type_id'],'is_show2'=>$_GPC['is_show2'])),'success');
	}else{
		message('设置失败','','error');
	}
}
if($_GPC['op']=='delete'){
	...
	if($result){
		message('删除成功',$this->createWebUrl2('dlgroupgoods',array()),'success');
	}else{
		message('删除失败','','error');
	}
}
include $this->template('web/dlgroupgoods');
