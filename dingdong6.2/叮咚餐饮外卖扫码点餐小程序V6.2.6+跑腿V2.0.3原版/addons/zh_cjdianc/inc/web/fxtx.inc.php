...
if($type=='all'){    
  $sql="SELECT * FROM ".tablename('cjdc_commission_withdrawal') ."".$where." ORDER BY time DESC";
  $total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('cjdc_commission_withdrawal') ."".$where." ORDER BY time DESC",$data);
}else{
...
if($operation=='adopt'){//审核通过
    $id=$_GPC['id'];
    $list=pdo_get('cjdc_commission_withdrawal',array('id'=>$_GPC['id']));
    $user=pdo_get('cjdc_user',array('id'=>$list['user_id']));
   
  $res=pdo_update('cjdc_commission_withdrawal',array('state'=>2,'sh_time'=>time()),array('id'=>$id));  
    if($res){
  ...
  $system=pdo_get('cjdc_system',array('uniacid'=>$_W['uniacid']));
   ...
   // $key=md5($key);
    ...
    if($return['result_code']=='SUCCESS'){
      pdo_update('cjdc_commission_withdrawal',array('state'=>2,'sh_time'=>time()),array('id'=>$id));
      message('审核成功',$this->createWebUrl('fxtx',array()),'success');
    }else{
...
if($operation=='reject'){
     $id=$_GPC['id'];
     $list=pdo_get('cjdc_commission_withdrawal',array('id'=>$id));
    $res=pdo_update('cjdc_commission_withdrawal',array('state'=>3,'sh_time'=>time()),array('id'=>$id));
     if($res){
        pdo_update('cjdc_user',array('commission +='=>$list['tx_cost']),array('id'=>$list['user_id']));
...
if($operation=='delete'){
     $id=$_GPC['id'];
     $res=pdo_delete('cjdc_commission_withdrawal',array('id'=>$id));
     if($res){
