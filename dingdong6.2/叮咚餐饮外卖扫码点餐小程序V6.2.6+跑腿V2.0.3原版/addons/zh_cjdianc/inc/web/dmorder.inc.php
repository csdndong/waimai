...
if(checksubmit('export_submit', true)) {
  ...
  $time="'%$time%'";
   $start=strtotime($_GPC['time']['start']);
  $end=strtotime($_GPC['time']['end']);
  $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("cjdc_order")." WHERE uniacid={$_W['uniacid']} and type=4 and dm_state=2 and  unix_timestamp(time) >={$start} and unix_timestamp(time)<={$end}");
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select a.*,b.name as md_name from " . tablename("cjdc_order")."  a"  . " inner join " . tablename("cjdc_store")." b on a.store_id=b.id  WHERE a.uniacid={$_W['uniacid']} and a.type=4 and unix_timestamp(a.time) >={$start} and a.dm_state=2 and unix_timestamp(a.time) <={$end} limit " . ($j - 1) * 5000 . ",5000 ";
