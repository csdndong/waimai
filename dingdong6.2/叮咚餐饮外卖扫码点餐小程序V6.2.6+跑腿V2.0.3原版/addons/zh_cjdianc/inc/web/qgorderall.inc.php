...
$sql="SELECT a.*,b.name as md_name,c.poundage as md_poundage,d.poundage,d.is_poundage,d.ps_mode FROM ".tablename('cjdc_qgorder'). " a"  . " left join " . tablename("cjdc_store") . " b on a.store_id=b.id " . " left join " . tablename("cjdc_storetype") . " c on b.md_type=c.id ". " left join " . tablename("cjdc_storeset") . " d on b.id=d.store_id ".$where." ORDER BY a.id DESC";
...
//打印
if($_GPC['op']=='dy'){
  $result=$this->rushPrint($_GPC['order_id']);
  //$result=$this->hcPrint($_GPC['order_id']);
  message('打印成功！', $this->createWebUrl('qgorderall'), 'success');
}
//print_R($list);die;
include $this->template('web/qgorderall');