...
if(checksubmit('export_submit', true)) {
  ...
  $time="'%$time%'";
   $start=$_GPC['time']['start'];
  $end=$_GPC['time']['end'];
  $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("cjdc_order")." WHERE uniacid={$_W['uniacid']} and type=1 and time >='{$start}' and time<='{$end}'");
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select a.*,b.name as md_name from " . tablename("cjdc_order")."  a"  . " inner join " . tablename("cjdc_store")." b on a.store_id=b.id  WHERE a.uniacid={$_W['uniacid']} and a.type=1 and a.time >='{$start}' and a.time<='{$end}' limit " . ($j - 1) * 5000 . ",5000 ";
