...
if(checksubmit('export_submit', true)) {
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select * from " . tablename("cjdc_order")." WHERE type=4 and store_id={$storeid} and pay_type in (1,2) and dm_state=2 and UNIX_TIMESTAMP(time) >='{$start}' and UNIX_TIMESTAMP(time)<='{$end}' ORDER BY id DESC limit " . ($j - 1) * 5000 . ",5000 ";
...
include $this->template('web/dldmzjlist');
