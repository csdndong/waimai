...
$sql2 = "select sum(tx_cost) as total from " . tablename("wpdc_withdrawal")." WHERE  store_id=".$storeid." and state in (1,2)";
...
message('提现金额小于最低提现金额','','error');
 ...
 if($res){
        message('提交成功',$this->createWebUrl2('dlintx',array()),'success');
    }else{
        message('提交失败','','error');
    }
...
include $this->template('web/dlintx');