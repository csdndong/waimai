...
if($_GPC['op']=='dy'){
  ...
  message('打印成功！', $this->createWebUrl('dlinydorder'), 'success');
}
...
if(checksubmit('export_submit', true)) {
  ...
  $end=$_GPC['time']['end'];
  $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("cjdc_order")." WHERE type=3 and store_id={$storeid} and yy_state!=1 and time >='{$start}' and time<='{$end}'");
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select a.*,b.name as md_name,c.name as table_name from " . tablename("cjdc_order")."  a"  . " inner join " . tablename("cjdc_store")." b on a.store_id=b.id inner join " . tablename("cjdc_table_type")." c on a.table_id=c.id WHERE a.type=3 and a.store_id={$storeid} and yy_state!=1 and a.time >='{$start}' and a.time<='{$end}' limit " . ($j - 1) * 5000 . ",5000 ";
