...
$sql="SELECT a.*,b.name as md_name,c.poundage as md_poundage,d.poundage,d.is_poundage,d.ps_mode,b.ps_poundage,u.name as userName FROM ".tablename('cjdc_order'). " a"  . " left join " . tablename("cjdc_store") . " b on a.store_id=b.id " . " left join " . tablename("cjdc_storetype") . " c on b.md_type=c.id ". " left join " . tablename("cjdc_storeset") . " d on b.id=d.store_id left join " . tablename("cjdc_user") . " u on u.id=a.user_id  ".$where." ORDER BY a.id DESC";
...
for($i=0;$i<count($list);$i++){
  $userorder=pdo_fetchcolumn("select count(*)  from " . tablename("cjdc_order") ." WHERE  store_id={$storeid} and user_id={$list[$i]['user_id']} and id<={$list[$i]['id']} and state!=1");
  $list[$i]['order_num']=$userorder;
  $data4=array();
...
if($_GPC['op']=='dy'){
  ...
  message('打印成功！', $this->createWebUrl('inorder'), 'success');
}
if($_GPC['op']=='cancel'){
    ...
    if ($result['result_code'] == 'SUCCESS' || $tkres) {//退款成功
      ...
      if($res){
        if($set['ps_mode']=='快服务配送'){
          ...
          message('取消成功！', $this->createWebUrl('inorder'), 'success');
        }
        if($set['ps_mode']=='达达配送'){
...
if($_GPC['op']=='wc'){
  ...
  if($res){
    ...
    message('完成成功！', $this->createWebUrl('inorder'), 'success');
  }else{
    message('完成失败！','','error');
  }
}
