...
function addLog($filename, $string)
{
    $s = '[' . date('Y-m-d H:i:s') . ']' . $string . PHP_EOL;
    file_put_contents(LOG_DIR . $filename, $s, FILE_APPEND);
}
    ...
    pdo_update('cjdc_groupgoods',array('ysc_num +='=>$grouporder['goods_num'],'inventory -='=>$grouporder['goods_num']),array('id'=>$grouporder['goods_id']));
        ...
        $group=pdo_get('cjdc_group',array('id'=>$grouporder['group_id']));
            if($group['kt_num']==$count['count']){
                $state=2;
                 getapi("https://".$_SERVER['HTTP_HOST'] . "/app/index.php?i=".$grouporder['uniacid']."&c=entry&a=wxapp&do=GroupPrint&m=zh_cjdianc&group_id=".$grouporder['group_id']);//拼团打印
                  getapi("https://".$_SERVER['HTTP_HOST'] . "/app/index.php?i=".$grouporder['uniacid']."&c=entry&a=wxapp&do=GroupMessage&m=zh_cjdianc&group_id=".$grouporder['group_id']);//拼团模板消息
            }else{
                $state=1;
            }
        //改变团状态
    ...
    if($order['type']==1 and ($order['state']==1 || $order['state']==6)){//外卖
           ...
           getapi("https://".$_SERVER['HTTP_HOST'] . "/app/index.php?i=".$order['uniacid']."&c=entry&a=wxapp&do=WeChatMsg&m=zh_cjdianc&id=".$order['id']);//模板消息
        ...
        $sms=getapi("https://".$_SERVER['HTTP_HOST'] . "/app/index.php?i=".$order['uniacid']."&c=entry&a=wxapp&do=Message&m=zh_cjdianc&order_id=".$order['id']);
    ...
    getapi("https://".$_SERVER['HTTP_HOST'] . "/app/index.php?i=".$order['uniacid']."&c=entry&a=wxapp&do=JsCommission&m=zh_cjdianc&order_id=".$order['id']);
        getapi("https://".$_SERVER['HTTP_HOST'] . "/app/index.php?i=".$order['uniacid']."&c=entry&a=wxapp&do=storeYyOrderMessage&m=zh_cjdianc&order_id=".$order['id']);//模板消息
         getapi("https://".$_SERVER['HTTP_HOST'] . "/app/index.php?i=".$order['uniacid']."&c=entry&a=wxapp&do=QtPrint&m=zh_cjdianc&order_id=".$order['id']);//
