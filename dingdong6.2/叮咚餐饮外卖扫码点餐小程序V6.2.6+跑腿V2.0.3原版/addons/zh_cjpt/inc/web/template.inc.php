<?php
global $_GPC, $_W;
// $action = 'ad';
// $title = $this->actions_titles[$action];
$GLOBALS['frames'] = $this->getMainMenu();
$operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
 $item=pdo_get('cjpt_message',array('uniacid'=>$_W['uniacid']));
    if(checksubmit('submit')){
          $data['appId']=trim($_GPC['appId']);
             $data['appSecret']=trim($_GPC['appSecret']);
            $data['jdId']=trim($_GPC['jdId']);
             $data['pdId']=trim($_GPC['pdId']);
            $data['uniacid']=trim($_W['uniacid']);
             $data['time']=time();
            if($_GPC['id']==''){                
                $res=pdo_insert('cjpt_message',$data);
                if($res){
                    message('添加成功',$this->createWebUrl('template',array()),'success');
                }else{
                    message('添加失败','','error');
                }
            }else{
                $res = pdo_update('cjpt_message', $data, array('id' => $_GPC['id']));
                if($res){
                    message('编辑成功',$this->createWebUrl('template',array()),'success');
                }else{
                    message('编辑失败','','error');
                }
            }
        }
    include $this->template('web/template');