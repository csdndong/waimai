...
Page({
    ...
    formSubmit: function(e) {
        ...
        if (console.log(o), 0 != o.length) {
            var i = getApp().getOpenId,
                s = this.data.store.name,
                l = this.data.store.id,
                n = this.data.StoreInfo.storeset.is_yydc,
                r = e.detail.value.datepicker,
                d = e.detail.value.timepicker,
                c = e.detail.value.lxr,
                u = e.detail.value.jcrs,
                p = e.detail.value.tel,
                f = e.detail.value.beizhu,
                g = e.detail.value.radioChange1;
            if (null != e.detail.value.zwpicker) var h = e.detail.value.zwpicker.name,
                m = e.detail.value.zwpicker.id,
                v = (e.detail.value.zwpicker.zd_cost, parseFloat(e.detail.value.zwpicker.yd_cost));
            else h = "", m = "", v = 0;
            if ("1" == n && "1" == g) var w = (v + Number(this.data.gwcprice)).toFixed(2);
            else w = v;
            if ("1" != g || 0 != this.data.cart_list.length) {
                ...
                var y = "",
                    z = !0;
                "" == c ? y = "请填写您的姓名！" : "" == u ? y = "请选择您的就餐人数" : "" == p ? y = "请填写您的手机号！" : p.length < 7 ? y = "手机号格式不正确" : (z = !1, t.setData({
                    showModal: !0
                })), 1 == z && wx.showModal({
                    title: "提示",
                    content: y
    ...
    qdzf: function(e) {
        var a = this,
            t = this.data.forminfo,
            o = this.data.zffs,
            i = getApp().getOpenId,
            s = this.data.store.name,
            l = wx.getStorageSync("users").id;
        if ("2" == o) {
            var n = Number(this.data.userInfo.wallet),
                r = Number(this.data.totalprice);
        ...
        var d = this.data.store.id,
            c = this.data.StoreInfo.storeset.is_yydc,
            u = this.data.date,
            p = this.data.time,
            f = t.lxr,
            g = t.jcrs,
            ...
            v = t.radioChange1,
            w = t.sexradiogroup;
        if (null != t.zwpicker) var y = t.zwpicker.name,
            z = t.zwpicker.id,
            _ = (t.zwpicker.zd_cost, parseFloat(t.zwpicker.yd_cost));
        else y = "", z = "", _ = 0;
        var x = [],
            D = this.data.cart_list,
            k = this.data.totalprice;
        "1" == v && D.map(function(e) {
            if (0 < e.num) {
                var t = {};
                t.name = e.name, t.img = e.logo, t.num = e.num, t.money = e.money, t.dishes_id = e.good_id, t.spec = e.spec, x.push(t)
            }
        }), console.log(k, D, x), console.log(i, s, l, d, u, p, y, z, f, g, h, _, m, v, c, w), console.log(t, form_id, o), this.setData({
            zfz: !0
        }), app.util.request({
    ...
    onLoad: function(e) {
        ...
        var a = this,
            t = e.storeid,
            o = util.formatTime(new Date).substring(0, 10).replace(/\//g, "-"),
