...
Page({
    ...
    onLoad: function(t) {
        ...
        null != t.drid && (i = "mycar2"), null == t.drid && (i = "mycar"), console.log(i), wx.removeStorageSync("note"), app.util.request({
            ...
            success: function(t) {
                console.log(t.data);
                for (var a = [], e = [], o = 0; o < t.data.length; o++) "1" != t.data[o].coupon_type && "1" == t.data[o].type && a.push(t.data[o]), "1" != t.data[o].coupon_type && "2" == t.data[o].type && e.push(t.data[o]);
    ...
    gettotalprice: function() {
        var t = this.data.gwcprice,
            a = this.data.mjmoney,
            e = this.data.xyhmoney,
            o = this.data.yhqkdje,
            i = this.data.hbkdje,
            s = (Number(a) + Number(e) + Number(o) + Number(i)).toFixed(2),
            n = (Number(t) - s).toFixed(2);
        n < 0 && (n = 0), console.log("gwcprice", t, "mjmoney", a, "xyhmoney", e, "totalyh", s, "totalPrice", n, "yhqkdje", o, "hbkdje", i), this.setData({
            totalyh: s,
            totalPrice: n,
            isloading: !1
        })
    ...
    countMj: function() {
        var t = this.data.gwcprice,
            a = this.data.reduction.reverse(),
            e = this.jsmj(t, a),
            o = this.data.isnewuser;
        console.log(t, a, e, o);
        var i = 0;
    ...
    formSubmit: function(e) {
        ...
        this.data.cart_list.map(function(t) {
            if (0 < t.num) {
                var a = {};
                a.name = t.name, a.img = t.logo, a.num = t.num, a.money = t.money, a.dishes_id = t.good_id, a.spec = t.spec, o.push(a)
            }
        }), console.log(o);
        var i = this,
            s = getApp().getOpenId;
        console.log("form发生了submit事件，携带数据为：", e.detail.value.radiogroup, this.data.activeradio, this.data.hbactiveradio);
        var a = this.data.tableid,
            n = e.detail.formId,
            d = this.data.form_id2,
            r = wx.getStorageSync("users").id,
            l = this.data.store.id,
            c = this.data.totalPrice,
            u = this.data.totalyh,
            h = this.data.gwcinfo.box_money,
            g = this.data.mjmoney,
            p = this.data.xyhmoney,
            f = parseInt(this.data.selectedindex) + 1,
            m = this.data.note,
            y = this.data.cjarray[this.data.cjindex],
            w = this.data.yhqkdje,
            v = this.data.activeradio,
            x = this.data.hbactiveradio,
            _ = this.data.hbkdje;
        if (console.log("桌子id", a, s, n, d, r, l, "实付", c, "总优惠", u, "包装费", h, "满减金额", g, "新用户money", p, "优惠券", w, "红包", _, "订单类型", f, "留言", m, "sz", o, "餐具数量", y), "yezf" == e.detail.value.radiogroup) {
            var b = Number(this.data.userInfo.wallet),
                D = Number(c);
