...
Page({
    ...
    sjradioChange: function(t) {
        ...
        for (var e = this.data.radioItems, a = 0, s = e.length; a < s; ++a) e[a].checked = e[a].id == t.detail.value, e[a].id == t.detail.value && this.setData({
    ...
    distance: function(t, e, a) {
        ...
        qqmapsdk.calculateDistance({
            ...
            success: function(t) {
                console.log(t), 0 == t.status && (s = Math.round(t.result.elements[0].distance), a(s))
            },
            fail: function(t) {
    ...
    tjddformSubmit: function(t) {
        ...
        var e = this.data.address,
            a = this.data.selectedindex,
            s = this.data.storeset;
    ...
    xzq: function(t) {
        if (console.log(t.currentTarget.dataset, this.data.gwcinfo.money, this.data.CouponSet.yhq_set), Number(t.currentTarget.dataset.full) > this.data.gwcinfo.money) return wx.showModal({
            title: "提示",
    ...
    onLoad: function(t) {
        ...
        wx.removeStorageSync("note"), app.util.request({
            ...
            success: function(t) {
                ...
                console.log(t, e.toString()), "" != t.data.dq_time && t.data.dq_time >= e.toString() && (t.data.ishy = 1), u.setData({
                    userInfo: t.data
                })
            }
        }), app.util.request({
            ...
            success: function(t) {
                console.log(t.data);
                for (var e = [], a = [], s = 0; s < t.data.length; s++) "2" != t.data[s].coupon_type && "1" == t.data[s].type && e.push(t.data[s]), "2" != t.data[s].coupon_type && "2" == t.data[s].type && a.push(t.data[s]);
            ...
            success: function(t) {
                ...
                var a = t.data,
                    e = t.data.store.coordinates.split(","),
                    s = {
                        lng: Number(e[1]),
                        lat: Number(e[0])
                    };
    ...
    gettotalprice: function() {
        var t = this.data.gwcprice,
            e = (this.data.gwcinfo.box_money, this.data.psf),
            a = this.data.mjmoney,
            s = this.data.xyhmoney,
            o = this.data.yhqkdje,
            i = this.data.hbkdje;
        var n = (Number(a) + Number(s) + 0 + Number(o) + Number(i)).toFixed(2);
        this.data.haveQg && (n = 0), (1 == this.data.userInfo.ishy && "1" == getApp().xtxx.is_vip_delivery || "0" != this.data.storeset.full_delivery && Number(t) - n >= Number(this.data.storeset.full_delivery || 1e5)) && (e = 0, this.setData({
            psf: 0,
            mpsf: !0
        }));
    ...
    formSubmit: function(a) {
        ...
        this.data.cart_list.map(function(t) {
            if (0 < t.num) {
                var e = {};
                e.name = "1" == t.is_qg ? t.qg_name : t.name, e.img = "1" == t.is_qg ? t.qg_logo : t.logo, e.num = t.num, e.money = t.money, e.dishes_id = t.good_id, e.spec = t.spec, e.is_qg = t.is_qg, s.push(e)
            }
        }), console.log(s);
        var o = this,
            i = this.data.storeset,
            t = this.data.haveQg,
            n = getApp().getOpenId;
        console.log("form发生了submit事件，携带数据为：", a.detail.value.radiogroup, this.data.activeradio, this.data.hbactiveradio);
        var e, d = a.detail.formId,
            r = this.data.form_id2,
            l = wx.getStorageSync("users").id,
            c = this.data.store.id,
            u = this.data.totalPrice,
            h = t ? 0 : this.data.totalyh,
            m = this.data.gwcinfo.box_money,
            g = this.data.psf,
            f = t ? 0 : this.data.mjmoney,
            p = t ? 0 : this.data.xyhmoney,
            y = this.data.note,
            w = this.data.cjarray[this.data.cjindex],
            v = t ? 0 : this.data.yhqkdje,
            _ = this.data.activeradio,
            x = this.data.hbactiveradio,
            b = t ? 0 : this.data.hbkdje,
            D = this.data.zkmoney;
        if ("hdfk" == a.detail.value.radiogroup && "3" == i.is_hdfk && Number(g) <= 0) wx.showModal({
            ...
            if ("2" == o.data.storeset.is_ps && (k = 2), e = 2 == k ? 0 < this.data.timearr.length ? this.data.xztime : this.data.date + " " + this.data.time : 0 < this.data.timearr.length ? this.data.xztime : this.data.wmtimearray[this.data.wmindex], null != this.data.address) var j = this.data.address.area.replace(/,/g, "") + this.data.address.address,
                z = this.data.address.user_name,
                q = this.data.address.tel,
                T = this.data.address.sex,
                S = this.data.address.area,
                M = this.data.address.lat,
                N = this.data.address.lng;
            else j = "", z = "", q = "", T = "0", S = "", M = "", N = "";
            if (2 == k && (z = o.data.name, q = this.data.mobile, "" == z || "" == q)) return wx.showModal({
                title: "提示",
                content: "到店自提必须填写收货人和联系电话！"
            }), !1;
            if (console.log(n, d, r, l, c, "实付", u, "总优惠", h, "包装费", m, "运费", g, "满减金额", f, "新用户money", p, "优惠券", v, "红包", b, "折扣", D, "订单类型", k, S, M, N, "收货人", z, "收获电话", q, "收货地址", j, "留言", y, "sz", s, "配送时间", e, "餐具数量", w, T), "yezf" == a.detail.value.radiogroup) {
                var C = Number(this.data.userInfo.wallet),
                    I = Number(u);
            ...
            console.log("支付方式", P), "" == d ? wx.showToast({
                ...
                success: function(t) {
                    ...
                    o.setData({
                        ...
                        title: "支付失败",
                        icon: "loading"
                    })), "hdfk" == a.detail.value.radiogroup && "1" == i.is_hdfk && (console.log("货到付款流程"), "下单失败" != e ? (o.setData({
                        mdoaltoggle: !1
                    }), setTimeout(function() {
                        wx.reLaunch({
                            url: "../wddd/order?status=2"
                        ...
                        title: "支付失败",
                        icon: "loading"
                    })), "hdfk" == a.detail.value.radiogroup && "3" == i.is_hdfk && (console.log("货到付款3流程"), "下单失败" != e ? app.util.request({
                        url: "entry/wxapp/pay",
                        cachetime: "0",
                        data: {
                            openid: n,
